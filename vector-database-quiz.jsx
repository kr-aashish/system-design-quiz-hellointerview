...
export const QUESTIONS = [
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Layer 3's greedy search will eventually reach every node in the graph through edge traversal, guaranteeing exact results"
      },
      {
        "label": "B",
        "text": "The hierarchical drop-down mechanism means Layer 3 narrows to the right region, and Layer 0's dense connectivity enables thorough local exploration to find the true nearest neighbor"
      },
      {
        "label": "C",
        "text": "HNSW maintains back-pointers from lower layers to upper layers, allowing the search to backtrack if a better candidate is found"
      },
      {
        "label": "D",
        "text": "Each layer independently runs a full KNN search, and results are merged across all layers to maximize recall"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "The similarity metric drifted because new vectors have different magnitude distributions than the original dataset"
      },
      {
        "label": "B",
        "text": "Newly inserted vectors have fewer and lower-quality graph connections compared to vectors present during the initial index build, degrading navigability"
      },
      {
        "label": "C",
        "text": "The HNSW index automatically reduced the number of layers to accommodate the larger dataset, reducing search precision"
      },
      {
        "label": "D",
        "text": "Memory pressure from the 50% growth caused the OS to swap index pages to disk, increasing latency and causing timeout-based result truncation"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "~120 GB — raw vectors are ~115 GB and HNSW overhead is minimal since it only stores edge lists as integer IDs"
      },
      {
        "label": "B",
        "text": "~240 GB — raw vectors are ~115 GB and HNSW roughly doubles memory due to the multi-layer graph structure storing edges at each layer"
      },
      {
        "label": "C",
        "text": "~60 GB — vector quantization is applied by default in HNSW implementations, reducing each dimension to 1 byte"
      },
      {
        "label": "D",
        "text": "~500 GB — each of the ~5 HNSW layers stores a complete copy of all vectors plus edge metadata"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Removing a node can disconnect the graph, creating isolated subgraphs where vectors become unreachable during search, catastrophically dropping recall for those regions"
      },
      {
        "label": "B",
        "text": "The deletion is O(n) because you must scan every edge in the graph to find and remove references to the deleted node"
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "Immediate deletion causes the layer promotion probabilities to become unbalanced, making upper layers too sparse for efficient navigation"
      }
    ],
    "correct": 0,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Pre-filter to the 60% matching products, then run exact KNN on the filtered set since it's small enough"
      },
      {
        "label": "B",
        "text": "Post-filter: retrieve top-5000 by vector similarity, then apply filters, hoping enough survive to fill the result page"
      },
      {
        "label": "C",
        "text": "Hybrid: run vector search with filtering integrated into the HNSW traversal, expanding exploration when filtered candidates are insufficient"
      },
      {
        "label": "D",
        "text": "Pre-filter to create a temporary HNSW index on the 3 million matching products, then run ANN search on that index"
      }
    ],
    "correct": 2,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "pgvector doesn't support HNSW, only IVF indexes, so the traversal mechanism is fundamentally different"
      },
      {
        "label": "B",
        "text": "pgvector relies on PostgreSQL's query planner, which for highly selective filters skips the vector index entirely and does brute-force similarity on the small filtered set using B-tree indexes first"
      },
      {
        "label": "C",
        "text": "The HNSW index pre-computes filtered results for common filter combinations, so it's using a cached result set"
      },
      {
        "label": "D",
        "text": "pgvector's HNSW implementation does perform filtered traversal, and the teammate's explanation is actually correct"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "ES needs to shard the vector index by filter dimensions, and without filter-specific shards, it falls back to scatter-gather across all shards"
      },
      {
        "label": "B",
        "text": "ES applies filters during HNSW traversal, and with only 0.1% match rate, the search must traverse vastly more of the graph to accumulate enough filtered results, dramatically increasing latency"
      },
      {
        "label": "C",
        "text": "ES's vector search doesn't support compound filters—it can only filter on a single field, so the three-field filter forces a full scan"
      },
      {
        "label": "D",
        "text": "The candidate embeddings are too high-dimensional for ES's kNN implementation, causing slow distance computations"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Hybrid search doubles the infrastructure cost since you need both a text search index and a vector index, and the marginal quality improvement rarely justifies the cost"
      },
      {
        "label": "B",
        "text": "For legal search, exact terminology matters enormously—'fiduciary' has precise legal meaning that semantic embeddings may dilute by returning conceptually similar but legally distinct results, making keyword search potentially superior alone"
      },
      {
        "label": "C",
        "text": "Hybrid search requires a ranking fusion algorithm to merge results, and no fusion algorithm can perfectly combine fundamentally different relevance signals without domain-specific tuning that may not generalize"
      },
      {
        "label": "D",
        "text": "Vector embeddings can't encode domain-specific legal terminology because embedding models are trained on general text corpora"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "~1,000 vectors — even small brute-force lists add measurable latency because each query must scan them entirely"
      },
      {
        "label": "B",
        "text": "~100,000 vectors — at this point the brute-force scan of the hot index takes comparable time to the HNSW search of the cold index, roughly doubling query latency"
      },
      {
        "label": "C",
        "text": "~10 million vectors — brute-force search is efficient with SIMD instructions, so it doesn't become a bottleneck until the hot index is very large"
      },
      {
        "label": "D",
        "text": "The hot index size doesn't matter because queries only search one index at a time based on the vector's insertion timestamp"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Hard deletes are slow (O(n) per delete) and will create a write bottleneck for the moderation pipeline"
      },
      {
        "label": "B",
        "text": "Hard deletes can fragment the HNSW graph by removing bridge nodes, causing recall degradation for vectors in the disconnected regions — soft deletes with periodic compaction is safer"
      },
      {
        "label": "C",
        "text": "The moderation system should delete from the primary database, not the vector index — the vector index should be treated as a derived view"
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Buffer inserts during the rebuild window and apply them all at once after the new index is live — the 4-hour delay in searchability is acceptable for most use cases"
      },
      {
        "label": "B",
        "text": "Maintain the hot index for new inserts during the rebuild. When the new cold index goes live, the hot index only contains 4 hours of vectors, keeping it small. Queries always search both."
      },
      {
        "label": "C",
        "text": "Use a streaming rebuild that processes new inserts into the new index as they arrive during the build process, ensuring zero delay"
      },
      {
        "label": "D",
        "text": "Partition the index into shards and rebuild one shard at a time, so only a fraction of vectors are briefly unavailable"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Pattern 1 (Vector DB as separate service): Send the query vector to the vector service, get back 1000 IDs, fetch full metadata from the primary DB, apply the reranking model"
      },
      {
        "label": "B",
        "text": "Pattern 2 (Hybrid search): Combine keyword search on video titles with vector similarity on embeddings, merge results with a ranking function"
      },
      {
        "label": "C",
        "text": "Pattern 3 (Two-stage retrieval): Vector search returns 1000 candidates, then a separate reranking model scores them using features the embedding doesn't capture (freshness, demographics, maturity)"
      },
      {
        "label": "D",
        "text": "Skip vector search entirely — use collaborative filtering on the user-item interaction matrix, which directly captures watch patterns without needing embeddings"
      }
    ],
    "correct": 2,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Yes — two-stage retrieval is always the highest quality approach, and 10K documents is small enough that the added latency is negligible"
      },
      {
        "label": "B",
        "text": "No — for 10K documents with a simple retrieval use case, Pattern 1 (vector DB as separate service) or even brute-force KNN is sufficient. The reranker adds latency and complexity without meaningful quality improvement at this scale."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "'Not necessarily. Vector search is typically eventually consistent — there may be seconds to minutes of delay before new embeddings are searchable. For this use case, I'd mention this as a known limitation and implement a write-through cache that checks recent uploads before vector search results.'"
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "At 2M vectors, pgvector or Elasticsearch kNN would suffice with far less operational complexity. Milvus is a distributed system with multiple node types — the operational overhead is unjustified at this scale and 'future-proofing' is premature optimization."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Always — custom models always outperform general-purpose embeddings because they're trained on domain-specific data"
      },
      {
        "label": "B",
        "text": "When the notion of 'similarity' is domain-specific and non-obvious — e.g., diapers and baby bottles are semantically dissimilar but should be similar in a 'frequently bought together' embedding space"
      },
      {
        "label": "C",
        "text": "Only when the product catalog exceeds 10 million items, because general-purpose embeddings lose precision at scale"
      },
      {
        "label": "D",
        "text": "When latency requirements are strict, because custom models can be optimized to produce lower-dimensional embeddings"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "At 100M vectors with 3072 dimensions in float32, raw vectors alone consume ~1.1 TB of RAM. With HNSW overhead (~2x), you need ~2.2 TB — a massive infrastructure cost that may not proportionally improve result quality compared to 384 or 768 dimensions."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "When vectors are normalized to unit length, cosine similarity and Euclidean distance produce identical rankings — but cosine similarity's bounded range [−1, 1] makes thresholds more interpretable and portable across datasets"
      },
      {
        "label": "C",
        "text": "Cosine similarity works in any number of dimensions while Euclidean distance becomes meaningless above 100 dimensions"
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Dot product is always incompatible with vector databases; you must convert to cosine similarity before indexing"
      },
      {
        "label": "B",
        "text": "Cosine similarity ignores vector magnitude, but the model may have learned to encode relevance strength in the magnitude — a 'very popular' product might have a larger vector. Switching to cosine could discard this signal."
      },
      {
        "label": "C",
        "text": "The dot product loss creates non-normalized vectors, and most vector databases require normalized inputs to function correctly"
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "IVF uses less memory per vector because it stores cluster assignments (a single integer) rather than graph edges, making it cheaper to scale"
      },
      {
        "label": "B",
        "text": "IVF inserts are cheaper — you just compute the nearest centroid and assign the vector to that cluster, avoiding the expensive graph connection operations HNSW requires"
      },
      {
        "label": "C",
        "text": "IVF indexes can be queried during construction, while HNSW indexes cannot serve queries until fully built"
      },
      {
        "label": "D",
        "text": "IVF automatically rebalances clusters as new data arrives, maintaining optimal performance without manual intervention"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "The 5 probed clusters always cover the same region of the vector space, creating systematic blind spots for certain query types"
      },
      {
        "label": "B",
        "text": "With nprobe=5, queries near cluster boundaries may miss relevant vectors in adjacent clusters. Queries whose true nearest neighbors span multiple clusters see degraded recall, while queries that land squarely within a single cluster work well."
      },
      {
        "label": "C",
        "text": "The k-means clustering has converged to a local minimum, creating uneven cluster sizes. Some clusters have millions of vectors and others have thousands."
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "HNSW — it has the best recall, and 256 dimensions is small enough that memory overhead is manageable"
      },
      {
        "label": "B",
        "text": "IVF — its fast inserts and low memory overhead make it ideal for streaming workloads at this scale"
      },
      {
        "label": "C",
        "text": "LSH — its O(1) hash-based inserts handle the extreme write rate, multiple hash tables provide robust recall, and the time-windowed nature means you can drop old hash buckets efficiently"
      },
      {
        "label": "D",
        "text": "Annoy — its memory-mapped index files are perfect for high-throughput workloads since multiple processes can share the index"
      }
    ],
    "correct": 2,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "With a single hash table, two similar vectors that happen to disagree on even one of the 16 hyperplane decisions end up in completely different buckets — recall will be unacceptably low because there's no redundancy to catch these near-misses"
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Annoy indexes are immutable — once built, you cannot add new vectors. A news feed with continuous article publications would require constant full index rebuilds, which is impractical."
      },
      {
        "label": "C",
        "text": "Annoy's random projection trees produce lower-quality results than HNSW for text embeddings specifically"
      },
      {
        "label": "D",
        "text": "Annoy requires all vectors to have exactly the same dimensionality, which limits which embedding models you can use"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "400 GB without mmap (50GB × 8 copies). With mmap, still 400 GB because each process needs its own copy of the data in physical RAM."
      },
      {
        "label": "B",
        "text": "400 GB without mmap. With mmap, ~50 GB total because the OS shares the same physical memory pages across all 8 processes, and the OS intelligently pages in only the actively-used portions."
      },
      {
        "label": "C",
        "text": "50 GB without mmap because modern OSes deduplicate memory. Mmap provides no additional benefit."
      },
      {
        "label": "D",
        "text": "400 GB without mmap. With mmap, 0 GB in RAM because the data stays entirely on disk and is accessed via I/O."
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Pinecone — it's fully managed and serverless, so your small team doesn't have to worry about infrastructure"
      },
      {
        "label": "B",
        "text": "pgvector — you're already on PostgreSQL, it handles millions of vectors fine, and you get joins with relational data, ACID transactions, and familiar tooling with zero new infrastructure"
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "Build a custom solution using FAISS in-memory — it gives you the most control and best performance"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "API design and query language — GraphQL support (Weaviate) vs REST (Pinecone) vs gRPC (Milvus) will determine developer productivity"
      },
      {
        "label": "B",
        "text": "Operational complexity vs scale — Pinecone is fully managed but offers less control, Milvus handles billions but requires operating a distributed system, and the team must decide how much infrastructure burden they can absorb"
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "Filtering capabilities — since 'People You May Know' requires complex demographic and social graph filters, the filtering implementation matters more than raw vector search performance"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "They used float16 math instead of float32. Actual: 1536 × 4 bytes = ~6 KB per vector, so 1 billion vectors = ~6 TB raw, plus index overhead"
      },
      {
        "label": "B",
        "text": "They forgot to account for metadata storage. The vector itself is 1.5 KB, but metadata doubles the per-vector cost"
      },
      {
        "label": "C",
        "text": "They're actually correct — 1536 × 1 byte per dimension = 1.5 KB, since vector databases compress to 8-bit integers by default"
      },
      {
        "label": "D",
        "text": "They divided by 1024 instead of 1000 when converting bytes to kilobytes, leading to a rounding error"
      }
    ],
    "correct": 0,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "No — HNSW search over 10M vectors typically takes 50-100ms, consuming the entire latency budget with nothing left for other operations"
      },
      {
        "label": "B",
        "text": "Yes — well-tuned HNSW search over 10M vectors typically achieves 1-5ms latency, leaving ample budget for the remaining operations"
      },
      {
        "label": "C",
        "text": "Only if you use IVF instead of HNSW, which achieves sub-millisecond latency at the cost of recall"
      },
      {
        "label": "D",
        "text": "Only if the index fits entirely in L3 cache — RAM-resident indexes at this size are too slow"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Vector databases are NOT transactional databases and should not be used as the source of truth. They lack ACID guarantees, can lose data during index rebuilds, and offer no relational integrity. The authoritative data must live in a proper database; the vector DB is an index."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Vector search finds SIMILAR things, not IDENTICAL things. Embedding 'DOC-4582' will return documents with semantically similar metadata, not the document with that exact ID. Exact lookups require a traditional database index."
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "This approach works fine; vector databases support exact match as a special case of similarity search with distance threshold 0"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Yes — brute force KNN over 50K × 768 dimensions involves ~150 million FLOPs per query, which modern CPUs with SIMD can complete in under a millisecond. The ANN index adds complexity with no meaningful latency improvement at this scale."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "'No — recall 0.92 means we find 92% of the true top-K nearest neighbors. The results we DO return are all genuinely similar; we just might miss some results that are slightly more similar than what we returned. We're not returning wrong answers — we're occasionally missing the absolute best ones.'"
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "v1 and v2 vectors live in incompatible embedding spaces. A v2 query vector will compute meaningless similarity scores against v1 vectors — they're geometrically unrelated even if the source data is similar. You must re-embed ALL existing data with v2."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Generate a random embedding for new users and let the vector search return results — random vectors will find random but potentially interesting items"
      },
      {
        "label": "B",
        "text": "Fall back to popularity-based or demographic-based recommendations for new users until enough behavioral signal accumulates to generate a meaningful embedding, then transition to vector-based recommendations"
      },
      {
        "label": "C",
        "text": "Pre-compute a 'default user' embedding as the average of all existing user embeddings and use it for cold start queries"
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "A single query can saturate a GPU, but under real load with thousands of concurrent queries, GPU compute becomes the bottleneck — you'd need many expensive GPUs. ANN indexes on CPU serve the same throughput at a fraction of the hardware cost."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "BM25 matches keywords, but duplicate tickets often describe the same issue with completely different words. 'My app keeps crashing on login' and 'Authentication screen freezes and force-closes' are near-duplicates that share zero keywords but have very similar embeddings."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Use HNSW for the 500M corpus (optimized for read performance at scale) and a flat brute-force index for the 200K trending set (fast to rebuild, trivially handles frequent updates at this size)"
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "Use Annoy for the 500M corpus (memory-mapped for efficiency) and LSH for the 200K trending set (handles streaming updates)"
      }
    ],
    "correct": 1,
    "confidence": null,
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "label": "B",
        "text": "Request more candidates than K from the HNSW search (e.g., K × 1.2), then apply the soft-delete filter, and return exactly K results from the surviving candidates. The over-fetch factor accounts for soft-deleted vectors."
      },
      {
        "label": "C",
        "text": "Maintain a separate count of soft-deleted vectors and subtract from the total before reporting result counts to the user"
      },
      {
        "label": "D",
