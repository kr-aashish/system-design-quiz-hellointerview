...
export const QUESTIONS = [
  ...
  },
  {
    ...
    "options": [
      ...
      "CRC32 is faster than MD5, causing Team B's ring to process more requests and create a thundering herd effect on certain nodes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "MD5 is cryptographically secure while CRC32 is not, and the security difference directly impacts load distribution fairness Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
  ...
  },
  {
    ...
    "options": [
      ...
      "The flaw is that you can't pre-compute key movements because the hash function is non-deterministic and produces different results each time Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "The approach works but only if you have fewer than 1000 keys — above that threshold, consistent hashing requires virtual nodes for any data movement Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "50% of data becomes unavailable because 2 out of 6 nodes failed, and consistent hashing distributes data proportionally across all nodes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
  ...
  },
  {
    ...
    "options": [
      ...
      "All three approaches are equally effective since they all redistribute the load from the hot key across multiple nodes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Writes are unaffected because each salted key is independent — you just write to whichever shard the hash selects, and reads already handle aggregation Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "The problem is that salted keys can't be updated atomically across nodes, but this is fine because hot keys are typically read-only (like event listings) Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
  ...
  },
  {
    ...
    "options": [
      "(1) Read replicas, (2) Read replicas, (3) Read replicas — replication solves all hot-key problems regardless of the access pattern Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "(1) Key-space salting for all three scenarios since it's the most general-purpose solution that works regardless of read/write patterns or predictability Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "(1) Adaptive rebalancing, (2) Read replicas, (3) Key-space salting — matching complexity of solution to complexity of problem Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "(1) Read replicas, (2) Read replicas, (3) Read replicas — replication solves all hot-key problems regardless of the access pattern Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "'The risk is minimal because consistent hashing only moves a small fraction of keys during rebalancing, so the impact is bounded regardless of load conditions.' Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "'The main risk is data loss during migration, which is why I'd ensure all data is replicated before starting any rebalancing operation. Once replication is confirmed, migration is safe.' Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "'That's why I'd avoid adaptive rebalancing entirely and use read replicas instead — they don't require any data movement and are simpler to reason about.' Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Modulo hashing can't support more than 8 servers because the hash function's output range limits the maximum number of shards Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      "It breaks down because hash functions don't distribute evenly across small moduli like 5, causing permanent load imbalance Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      ...
      "It breaks down when data volume exceeds what 5 shards can handle, but that's a capacity planning failure, not a hashing algorithm problem Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "It breaks down because hash functions don't distribute evenly across small moduli like 5, causing permanent load imbalance Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      "Data movement only happens when a client next tries to read from the decommissioned node — at that point, the read is redirected and the key is lazily re-replicated Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "No data movement occurs because the data already exists on 2 other nodes, and RF=3 just means '3 nodes can serve it' — when one is removed, RF automatically adjusts to 2 Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Data movement is triggered immediately when the node goes offline, as the other 2 replicas race to copy data to a fresh node to maintain RF=3 within the heartbeat timeout Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Data movement only happens when a client next tries to read from the decommissioned node — at that point, the read is redirected and the key is lazily re-replicated Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "(1) Pause all writes, (2) Add new node and recalculate ring, (3) Redistribute data, (4) Resume writes — write pause ensures consistency during migration Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "(1) Add new node with empty data, (2) Immediately start routing requests to it, (3) Handle cache misses by redirecting to old owners, (4) Eventually the new node warms up via normal traffic Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
  ...
  },
  {
    ...
    "options": [
      ...
      "Redis's fixed hash slots work identically to consistent hashing — adding a node automatically causes 1/N of slots to migrate. The only difference is the fixed ring size of 16,384 instead of 2^32 Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Redis's approach causes the same full redistribution problem as modulo hashing because changing node count requires remapping all 16,384 slots Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Redis's approach causes the same full redistribution problem as modulo hashing because changing node count requires remapping all 16,384 slots Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      "Consistent hashing is worse because it sends all traffic for the viral video to one server, creating a hot spot, while round-robin naturally distributes the load Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Round-robin is better for viral content because it spreads the load evenly, preventing any single edge server from being overwhelmed by the hot video Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Both perform identically for cache hit rates because CDNs replicate popular content to all edge servers regardless of routing strategy Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Consistent hashing is worse because it sends all traffic for the viral video to one server, creating a hot spot, while round-robin naturally distributes the load Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Skip consistent hashing entirely and say 'DynamoDB handles it automatically' — any mention of internals shows you're not focused on the actual design problem Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
  ...
  },
  {
    ...
    "options": [
      ...
      "The candidate is correct — a smaller ring saves memory and computation. The 2^32 standard is a historical artifact from when hash functions only produced 32-bit outputs Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "A smaller ring would work identically because the hash function's modulo operation normalizes any ring size to produce uniform distribution, regardless of the ring's absolute size Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "The large ring is needed because consistent hashing must reserve space for future node additions — with 4.3 billion positions, you can add nodes for decades without running out of positions Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Fixed hash slots are better because they support up to 16,384 nodes while consistent hashing has practical limits around 50 nodes due to virtual node overhead Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Either approach works equally well — the ops team size is irrelevant because both approaches are fully automated in production implementations Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Fixed hash slots are better because they support up to 16,384 nodes while consistent hashing has practical limits around 50 nodes due to virtual node overhead Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "The candidate is correct — doubling virtual nodes will spread the hot key's traffic across more physical nodes since the key might hash to a different node's virtual node range Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "The mistake is not doubling virtual nodes enough — they should use 1000+ virtual nodes per physical node to achieve truly even distribution at the individual key level Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "The issue is that 100 virtual nodes was already sufficient — the hot spot is caused by a poor hash function, not insufficient virtual nodes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "'If DynamoDB doesn't use consistent hashing, then we shouldn't mention it in the context of DynamoDB. I'll redesign my answer to avoid the term entirely.' Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "'The distinction doesn't matter in an interview. Consistent hashing is consistent hashing — implementation details of cloud services are irrelevant to system design discussions.' Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Virtual nodes would have prevented the failure entirely by providing redundancy — if one of B's virtual nodes fails, the others keep serving Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "With virtual nodes, B's failure would move only 1/5 of B's keys instead of all of them, since each virtual node holds an independent portion Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "This is correct for the short term but creates a permanent imbalance — the overloaded node will have fewer keys than its peers indefinitely, wasting capacity when the hot key eventually cools down Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "This works but is suboptimal — the better approach is to increase virtual nodes on OTHER nodes, which achieves the same redistribution without touching the overloaded node's configuration Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "This is correct for the short term but creates a permanent imbalance — the overloaded node will have fewer keys than its peers indefinitely, wasting capacity when the hot key eventually cools down Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Nothing changes for key P's replicas because replication assignments are fixed at write time and don't update when the ring topology changes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "All 3 existing replicas remain, and Node N becomes a 4th replica temporarily until a background process removes the extra copy, effectively increasing RF to 4 during transition Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Node N takes over as the primary for key P but the replica set stays the same — only the primary assignment uses consistent hashing, not replica placement Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Stop all traffic, run a batch job to copy every key from its modulo-hashed location to its consistent-hashing location, then switch all clients at once. This is the only way to guarantee consistency Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Use consistent hashing immediately for all writes, but read from both the old and new locations, returning whichever version is newer. This naturally migrates data over time as keys are updated Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Configure the consistent hash ring positions to exactly match the modulo hash output, making both schemes route identically during transition Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      ...
      "Add more Cassandra nodes — consistent hashing with virtual nodes will automatically spread the hot partition's traffic across the new nodes Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Pre-split the hot partition into 10 sub-partitions to distribute reads across 10 nodes. This is the canonical Cassandra approach for hot partitions Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic.",
      "Do nothing — Cassandra's built-in read repair and speculative retry mechanisms can handle 10x read traffic spikes automatically Furthermore, this naive implementation fundamentally ignores core distributed systems principles, leading directly to catastrophic network partition failures and unacceptable replication lag under peak traffic."
    ],
    "correct": 0,
