...
export const QUESTIONS = [
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "A single U-24tb1.metal instance provides 24 TB of RAM, eliminating distributed coordination overhead, network serialization costs, and partial-failure complexity for this dataset size."
      },
      {
        "label": "B",
        "text": "20 servers with 1 TB each is optimal because distributed systems always outperform vertical scaling due to parallel CPU cores across machines."
      },
      {
        "label": "C",
        "text": "The 18 TB requirement itself is flawed — no real-world analytics workload needs that much in-memory data, so the candidate should challenge the requirement first."
      },
      {
        "label": "D",
        "text": "AWS instances max out at 4 TB of RAM (X1e.32xlarge), so the distributed approach is the only viable option and the candidate is correct."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Use i3en.24xlarge instances with 60 TB local SSD for hot data, and tier cold data to S3 with lifecycle policies. Total local SSD across 4 instances covers the hot tier with room for growth."
      },
      {
        "label": "B",
        "text": "Use D3en.12xlarge instances with 336 TB HDD each — a single instance holds everything, and HDD latency is acceptable for time-series workloads since they're sequential reads."
      },
      {
        "label": "C",
        "text": "Store everything on a single i3en.24xlarge with 60 TB SSD and use S3 for overflow. The 60 TB covers the hot 20 TB and most of the cold data."
      },
      {
        "label": "D",
        "text": "Use S3 for everything with CloudFront caching — object storage is unlimited and the CDN handles latency for frequently accessed data."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "50M × 2KB = 100 GB total data. A single Aurora instance supports up to 256 TiB storage and 50k read TPS. The data fits trivially, and 8k QPS is well within single-instance capacity — sharding adds unnecessary complexity."
      },
      {
        "label": "B",
        "text": "The candidate should use 8 shards, not 4 — with 50M products, you need at least one shard per 6.25M products to maintain index efficiency."
      },
      {
        "label": "C",
        "text": "The flaw is not sharding but missing a caching layer — 8k QPS always requires Redis in front of any database regardless of database capabilities."
      },
      {
        "label": "D",
        "text": "The candidate is correct to shard because 50 million rows will cause index bloat that degrades query performance below the 10ms p99 requirement on any single instance."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "The 6-hour backup windows and operational impact on replication lag — this is an operational concern that sharding directly addresses by reducing per-shard data volume."
      },
      {
        "label": "B",
        "text": "The 40 TiB data size alone — this exceeds the recommended sharding threshold of 50 TiB, so sharding is premature until you hit that limit."
      },
      {
        "label": "C",
        "text": "The 12k write TPS — this exceeds the 10k TPS single-instance threshold, making write throughput the primary driver."
      },
      {
        "label": "D",
        "text": "None of these justify sharding yet — optimize with read replicas, connection pooling, and query tuning first."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "~210 GB total (30M × 1KB + 6B × 3KB ≈ 18 GB + 180 GB ≈ ~200 GB). At this size, nothing in the database breaks — the bottleneck will be read throughput on popular posts (hot partitions), not storage or write capacity."
      },
      {
        "label": "B",
        "text": "~18 TB total. The storage is fine for a single instance, but write amplification from indexing 6 billion posts will exceed disk I/O capacity first."
      },
      {
        "label": "C",
        "text": "~200 GB total. The first thing that breaks is connection limits — 30 million users will generate concurrent connection counts exceeding the 20k limit."
      },
      {
        "label": "D",
        "text": "~200 GB total, but the candidate is right to shard because 6 billion rows will cause query planning overhead that degrades all queries, even indexed ones."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "'Single instance doesn't mean single point of failure. I'd run a primary with synchronous replication to a standby in another AZ for automatic failover. Aurora's multi-AZ does this natively. Replication for HA is separate from sharding for scale — we need the former, not the latter.'"
      },
      {
        "label": "B",
        "text": "'You're right — we should shard the database across 3 instances to eliminate the single point of failure. Each shard handles a third of the data and provides redundancy.'"
      },
      {
        "label": "C",
        "text": "'The single primary is fine because modern SSDs have a mean time between failures of 2+ million hours, making hardware failure statistically negligible.'"
      },
      {
        "label": "D",
        "text": "'We should use a multi-master setup like CockroachDB to ensure every write goes to multiple nodes simultaneously, eliminating the single point of failure.'"
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "A single large ElastiCache Redis instance: 200M × 500B = 100 GB fits in memory, 150k ops/sec is within the 100-200k ops/sec range, and sub-1ms read latency satisfies the 5ms SLA with margin."
      },
      {
        "label": "B",
        "text": "A Redis cluster with 10 shards: 100 GB needs distribution because Redis becomes inefficient above 10 GB, and 150k ops/sec requires sharding for throughput."
      },
      {
        "label": "C",
        "text": "Skip the cache entirely — a database with proper indexing can handle 150k simple key lookups per second with sub-5ms latency."
      },
      {
        "label": "D",
        "text": "Use application-server local caches: with 200M users × 500B = 100 GB per server, each app server keeps a full copy in its 512 GB RAM, eliminating network hops entirely."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Engineer B is correct: 500 GB fits on a single instance, the 'cache everything' approach eliminates cache miss thundering herds, and the $5k/month is far less than the engineering time to build, test, and maintain selective eviction logic."
      },
      {
        "label": "B",
        "text": "Engineer A is correct: selective caching is always more cost-effective because you only pay for the memory you actually need, and LRU is a well-understood algorithm with minimal engineering overhead."
      },
      {
        "label": "C",
        "text": "Neither is correct: 500 GB should be sharded across 5 Redis instances at 100 GB each for optimal performance, since Redis performance degrades significantly above 100 GB."
      },
      {
        "label": "D",
        "text": "Engineer B is correct but for the wrong reason: the real benefit is that caching everything eliminates the need for a database entirely, not just the eviction logic."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Network bandwidth saturation on the Redis instance — at 180k ops/sec, if average response payloads are large (e.g., 10+ KB), you can saturate the 25 Gbps network limit before hitting CPU or memory limits."
      },
      {
        "label": "B",
        "text": "Memory is the bottleneck at 60% — Redis needs 40% free memory for copy-on-write during background saves, and BGSAVE operations are causing the latency spikes."
      },
      {
        "label": "C",
        "text": "The 95% hit rate means 5% of requests are cache misses hitting the database, and those slow responses are inflating the latency percentiles."
      },
      {
        "label": "D",
        "text": "Redis is single-threaded, so at 180k ops/sec you're likely CPU-bound. The latency spikes occur when expensive operations (like large key scans or Lua scripts) block the event loop."
      }
    ],
    "correct": 3,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "The database already returns sub-5ms for indexed lookups, and 5k reads/sec is well within database capacity (50k TPS). Adding a cache layer introduces invalidation complexity, potential stale data, and operational overhead for a marginal latency improvement that likely doesn't affect user experience."
      },
      {
        "label": "B",
        "text": "Nothing is wrong — caching always improves performance, and the latency reduction from 3ms to sub-1ms is a 3x improvement that compounds across the request path."
      },
      {
        "label": "C",
        "text": "The issue is that Redis should be used for complex queries, not simple key-value lookups. For simple lookups, a local in-memory HashMap on the application server is more appropriate."
      },
      {
        "label": "D",
        "text": "The candidate should use Memcached instead of Redis for simple key-value lookups, as Redis's data structure overhead makes it slower than Memcached for this use case."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Cross-AZ latency is 1-2ms, not 50ms (that's cross-region). The candidate's overestimate leads them to avoid multi-AZ deployments, sacrificing availability for a latency penalty that doesn't actually exist. A two-phase commit with 3 AZ participants adds ~4-6ms, not 150ms."
      },
      {
        "label": "B",
      ...
      {
        "label": "C",
        "text": "Cross-AZ latency is sub-1ms (same as intra-AZ), so the candidate is overestimating by 50x, but this doesn't materially affect the design since the transaction logic dominates."
      },
      {
        "label": "D",
        "text": "The exact latency doesn't matter because distributed transactions should always use asynchronous eventual consistency, never synchronous two-phase commit."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "8 instances is sufficient — each instance supports 100k+ concurrent connections, and at 30% CPU utilization there's headroom. Scale to 8 for the connections plus reasonable overhead, not 20 based on current utilization."
      },
      {
        "label": "B",
        "text": "20 instances is correct — you should never run above 50% of connection capacity to handle traffic spikes, so 400k / 50k × 2 = 16, rounded up to 20."
      },
      {
        "label": "C",
        "text": "4 instances is sufficient — since each can handle 100k connections and CPU is the bottleneck at 30%, you have more than enough headroom."
      },
      {
        "label": "D",
        "text": "The number of instances doesn't matter — use a load balancer with connection pooling to multiplex 400k client connections into a smaller number of backend connections."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Horizontal scaling — add more instances. CPU is the bottleneck at 75% (above the 70% scaling threshold), and since memory and network are underutilized, vertical scaling to a larger instance would waste those resources. Adding instances distributes CPU load efficiently."
      },
      {
        "label": "B",
        "text": "Vertical scaling — move to a 128-core instance. Doubling cores is simpler than distributing image processing across instances and avoids the complexity of job distribution."
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "The bottleneck is actually memory, not CPU — image processing requires loading full images into RAM, and at 15% of 512 GB you're likely swapping."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "A single Kafka broker handles up to 1M messages/sec with sub-5ms latency. 500k msgs/sec at 2KB each is ~1 GB/sec throughput — well within network capacity of 25 Gbps. A single broker suffices for throughput, though you'd want replication for durability."
      },
      {
        "label": "B",
        "text": "500k messages/sec requires at least 5 Kafka brokers — each broker is limited to ~100k messages/sec in practice, so you need 5x capacity."
      },
      {
        "label": "C",
        "text": "Kafka is the wrong choice for notifications — it's designed for log aggregation, not transient messages. Use RabbitMQ or SQS for notification workloads."
      },
      {
        "label": "D",
        "text": "The calculation shows 500k × 2KB = 1 GB/sec, which exceeds a single broker's 50 TB storage. You'd fill the storage in under 14 hours and need to scale."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "'With modern queue latency at 1-5ms end-to-end within a region, we can include a Kafka produce step in a synchronous request while staying under typical 100-200ms API SLA budgets. This gives us reliable delivery and decoupling without forcing the API to be async — as long as there's no backlog.'"
      },
      {
        "label": "B",
        "text": "'You're right — I'll switch to a synchronous RPC call for reliability instead of a queue. Queues inherently add seconds of latency that make them incompatible with sync flows.'"
      },
      {
        "label": "C",
      ...
      {
        "label": "D",
        "text": "'The queue is a fire-and-forget pattern here — the API returns immediately without waiting for the queue acknowledgment, so latency isn't affected.'"
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Add read replicas — the read-to-write ratio is ~4:1, and offloading reads to replicas reduces primary load without sharding complexity. The 30k read TPS is within single-instance limits (50k) but adding replicas provides headroom and improves availability."
      },
      {
        "label": "B",
        "text": "Shard the database — at 25 TiB and 8k write TPS, you're approaching both the storage (50 TiB) and write (10k TPS) thresholds simultaneously."
      },
      {
        "label": "C",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Sharding (horizontal partitioning) distributes data across nodes for SCALE, not availability. For HA, you need replication — a primary with standby replicas for automatic failover. Sharding without replication on each shard still has single points of failure. These are orthogonal concerns."
      },
      {
        "label": "B",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Total data is 10M × 1KB + 100M × 5KB = 10 GB + 500 GB = ~510 GB. This fits trivially on a single database instance (capacity: 64+ TiB). Sharding adds cross-shard query complexity for searches spanning regions, distributed join overhead, and operational burden — all for 0.8% of a single instance's storage capacity."
      },
      {
        "label": "B",
        "text": "Geographic sharding is wrong because business queries aren't geographically bounded — users search across regions. But sharding by business ID would be correct at this scale."
      },
      {
        "label": "C",
        "text": "The flaw is in the shard key — geographic sharding creates hot spots in popular cities. The data size justifies sharding, but the key should be a hash of the business ID."
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Simple indexed key/row lookups on modern databases (PostgreSQL, MySQL, Aurora) return in 1-5ms for cached data. The candidate is overestimating database latency by 10-20x, leading them to add infrastructure that provides minimal real benefit. Caching is valuable for expensive queries, not simple indexed lookups."
      },
      {
        "label": "B",
      ...
      {
        "label": "C",
        "text": "The misconception is about Redis latency, not database latency. Redis actually provides 5-10ms latency, not sub-1ms, so the improvement is smaller than they think."
      },
      {
        "label": "D",
        "text": "The candidate is right to cache, but for the wrong reason. The real benefit of caching isn't latency reduction but protecting the database from connection exhaustion under high concurrency."
      }
    ],
    "correct": 0,
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "Partially valid — complex multi-table transactions with triggers significantly reduce effective write capacity below the 20k+ simple-write baseline. But the correct first step is to simplify the write path (denormalize, batch related updates, defer cascading logic asynchronously) rather than adding infrastructure to buffer inherently expensive writes."
      },
      {
        "label": "B",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "100k × 100k × (36B + 4B) = 400 GB. A single memory-optimized Redis instance supports up to 1 TB. The entire leaderboard dataset fits on one instance without sharding. Sharding adds routing complexity, cross-shard ranking challenges, and operational overhead for a dataset well within single-instance capacity."
      },
      {
        "label": "B",
      ...
      {
        "label": "C",
        "text": "The dataset is actually 100k × 100k × 40B = 400 TB, far exceeding even sharded cache capacity. The candidate should use a database with sorted indexes instead."
      },
      {
        "label": "D",
  ...
  },
  {
    ...
    "options": [
      {
        "label": "A",
        "text": "200k rides × 0.5 updates/sec = 100k WPS. This exceeds PostgreSQL's 20k WPS capacity for simple writes, so Kafka buffering is actually justified here. However, the candidate should also consider: (1) batching GPS writes, (2) writing to a time-series DB optimized for this pattern, or (3) reducing write frequency. Kafka alone just delays the bottleneck if the DB can't keep up sustainably."
      },
      {
        "label": "B",
