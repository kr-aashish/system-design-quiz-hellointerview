...
export const QUIZ_DATA = {
  ...
  "subtopicBreakdown": {
    "heavy": [
      ...
      {
        "name": "Delta Encoding & Compression",
        "count": 4
      },
      {
        "name": "Data Model (Tags vs Fields)",
        "count": 4
      },
      {
        "name": "Cardinality Problem",
        "count": 3
      }
    ],
    "medium": [
    ...
    "thin": [
      {
        "name": "Block-Level Metadata",
        "count": 1
      },
      {
        "name": "When NOT to use a TSDB",
        "count": 1
      },
      {
        "name": "Postgres Limitations + TSDB vs GP",
        "count": 1
      }
    ]
  },
  "questions": [
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Append-only storage eliminates the need for write-ahead logging, saving one disk write per operation"
        },
        {
          "label": "B",
          "text": "Sequential I/O avoids disk seek time — instead of random seeks to update rows in place, all writes go to the end of the file in order, which both spinning disks and SSDs handle dramatically faster"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "The storage engine can batch writes into larger blocks because append-only files don't need per-row locking"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Append-only storage makes writes fast but creates a read problem — data isn't organized for efficient lookups, requiring additional structures like LSM trees to reorganize data in the background"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "Append-only storage only works with time-series data because it relies on timestamps for ordering"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "C",
          "text": "When the memtable flushes to disk as an SSTable, it must produce a sorted file — a hash map would require a full sort at flush time, making flushes expensive and unpredictable instead of a simple sequential write of already-sorted data"
        },
        {
          "label": "D",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Bloom filters become less accurate with more SSTables, causing excessive false positives. Fix: increase the Bloom filter size per SSTable"
        },
        {
          "label": "B",
          "text": "Each SSTable's Bloom filter must still be checked in memory, and with 200 SSTables the cumulative false positive rate means many unnecessary disk reads. Fix: trigger compaction to merge SSTables, reducing the number of files to check"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "SSTables have overlapping key ranges, so binary search can't narrow down which file contains the key. Fix: enable range-partitioned compaction (leveled compaction)"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Compaction rewrites data multiple times (write amplification), competing for I/O. Mitigate by rate-limiting compaction throughput and accepting temporarily higher read latency from more SSTables"
        },
        {
          "label": "B",
          "text": "Compaction is CPU-bound from sorting merged data. Mitigate by using more CPU cores for parallel compaction"
        },
        {
          "label": "C",
          "text": "Compaction locks the SSTables being merged, blocking reads. Mitigate by using copy-on-write snapshots during compaction"
        },
        {
          "label": "D",
          "text": "Compaction generates too many temporary files. Mitigate by increasing the compaction threshold so it runs less frequently with larger batches"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "C",
          "text": "LSM trees trade read performance for write throughput — range scans may need to merge results across multiple SSTables, adding latency. A B-tree or columnar store would be more appropriate for this read-heavy, scan-intensive workload"
        },
        {
          "label": "D",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Variable-length encoding (varint) represents small numbers with fewer bytes — the number 1 might take 1 byte instead of 8. Converting large absolute values into small deltas enables varint to dramatically reduce per-value storage"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "Modern file systems automatically compress blocks with repeating byte patterns, so smaller numbers compress better at the filesystem level"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Delta-of-delta exploits regularity — when the interval is constant (10s), the delta-of-deltas are all zero, compressible to ~1 bit/value. Irregular intervals produce non-zero delta-of-deltas that require more bits, degrading compression from ~1 bit to several bytes per timestamp"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "When values cross a power-of-2 boundary (e.g., 63.9 → 64.1), the IEEE 754 exponent bits change, producing many differing bits in the XOR despite a small numerical delta"
        },
        {
          "label": "B",
        ...
        {
          "label": "C",
          "text": "When values alternate between positive and negative — the sign bit flip causes maximum XOR difference"
        },
        {
          "label": "D",
          "text": "When values have more than 6 decimal places of precision, exceeding the mantissa capacity"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The 1.37 bytes/value figure only covers the value — timestamps, tags, series keys, block metadata, and index overhead aren't included. Actual storage with metadata is 3-5x the compressed value size"
        },
        {
          "label": "B",
          "text": "Compression ratios degrade over time as data becomes less correlated, so older data takes more space per point"
        },
        {
          "label": "C",
          "text": "5 years of full-resolution data means 7.8 trillion data points — even at 1.37 bytes, query performance over that volume degrades catastrophically without downsampling because every query must scan enormous time ranges"
        },
        {
          "label": "D",
          "text": "Disk I/O bandwidth, not storage capacity, is the bottleneck — reading 5 years of data requires decompressing each block sequentially"
        }
      ],
      "correct": 2,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The query spans two daily partitions (yesterday 11:30 PM to today 1:30 AM), so the database must open and scan two partition files instead of one, doubling I/O"
        },
        {
          "label": "B",
          "text": "The overnight partition hasn't been compacted yet, so it contains many small SSTables"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "Night-time data has lower cardinality (fewer active hosts), making the Bloom filter less effective"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The database marks old partitions as 'expired' in metadata but keeps the files on disk for eventual garbage collection by the OS"
        },
        {
          "label": "B",
          "text": "The database runs DELETE statements optimized with partition pruning, which is faster than full table scans but still requires row-by-row processing"
        },
        {
          "label": "C",
          "text": "Old partitions are entire files (or directories) that can be dropped atomically — no row scanning, no index cleanup, no transaction log entries. It's essentially a filesystem delete operation"
        },
        {
          "label": "D",
          "text": "The database compresses old partitions into archive format, reducing their storage footprint instead of deleting them"
        }
      ],
      "correct": 2,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Exactly 1 — the Bloom filter for the correct SSTable says 'maybe here,' and all others say 'definitely not'"
        },
        {
          "label": "B",
          "text": "About 1.5 — the 1 true hit plus ~0.5 expected false positives from the 49 non-matching SSTables (49 × 0.01 ≈ 0.49)"
        },
        {
          "label": "C",
          "text": "About 5 — Bloom filters only reduce reads by 90% with 1% false positive rate"
        },
        {
          "label": "D",
          "text": "About 25 — Bloom filters cut reads in half regardless of the false positive rate"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Bloom filters only work with exact key lookups, not range predicates. Time-range queries don't check for a specific key — they scan all data within a time window. Time-based partitioning already handles the time dimension efficiently"
        },
        {
          "label": "B",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "The 95% spike lasted less than 1 minute. The 1-minute rollup stored min/max/avg/count, but this particular rollup only stored the average (78.3%), losing the peak. The rollup should store max values separately"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The database checks each block's max value — blocks where max < 90% are skipped entirely without decompressing. Limitation: blocks with max ≥ 90% must be fully scanned even if only one point exceeded 90%, potentially reading blocks where the actual spike is a tiny fraction of the data"
        },
        {
          "label": "B",
        ...
        {
          "label": "D",
          "text": "The min/max metadata is used to sort blocks by value range for binary search. Limitation: blocks can't be binary-searched because they're time-ordered"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Sharding Postgres across multiple instances for write throughput, adding read replicas for query speed, and normalizing the schema to reduce duplication"
        },
        {
          "label": "B",
          "text": "Append-only storage + LSM trees (eliminates index-degraded writes), columnar series-oriented storage with tag indexing (co-locates related data for fast scans), and delta/XOR compression (eliminates redundant storage of repeated metadata)"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "Adding a caching layer (Redis) for recent queries, partitioning Postgres by time, and enabling columnar compression extensions"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "An inverted index — it maps each tag value to a set of series IDs, and multi-tag queries intersect these sets. This is the same structure Elasticsearch uses, but instead of mapping words to documents, it maps tag values to time series"
        },
        {
          "label": "C",
        ...
        {
          "label": "D",
          "text": "A bitmap index — each tag value gets a bit vector where each bit represents a series, and AND/OR operations combine filters"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Request IDs are high-cardinality — millions of unique values per day. Since tags are indexed in memory, this creates millions of series entries in the tag index, exhausting memory and degrading query performance for all metrics, not just request-correlated ones"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The existing series is updated with the new tag — all future data points join the original series with an additional tag attribute"
        },
        {
          "label": "B",
          "text": "A completely new series is created (cpu_usage,host=server-1,region=us-west,env=prod,datacenter=dc1). The old series still exists with its historical data. Queries spanning the tag change must union results from both series"
        },
        {
          "label": "C",
          "text": "The tag is added to the index but doesn't change the series identity — both old and new data points belong to the same series"
        },
        {
          "label": "D",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Tag — you'll frequently filter by percentile ('show me all p99 latencies'), and there are only 3 distinct values (p50, p95, p99), so cardinality impact is minimal. The tag index enables efficient lookups"
        },
        {
          "label": "B",
          "text": "Field — percentiles are computed values that should be stored alongside the raw latency data, not used as dimensional metadata"
        },
        {
          "label": "C",
          "text": "Neither — store separate measurements: http_latency_p50, http_latency_p95, http_latency_p99, each as its own metric"
        },
        {
          "label": "D",
          "text": "Tag — but only if the cluster has enough memory to handle the additional series created by the percentile dimension"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "10 million unique user_id tags create 10 million+ series. The in-memory tag index for all user_id values consumes tens of gigabytes of RAM, and each write creates or updates a series entry. The TSDB becomes slower than Postgres for this workload"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "The series count grows unboundedly — every pod restart creates new series that are never cleaned up (the old pod's series still exist in the index). Adding memory only delays the inevitable OOM. The fix is to stop using ephemeral pod names as tags and use stable identifiers like deployment name or service name instead"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "The query reads from the buffer (most recent data) and disk files (older data) concurrently, merging results. Aggregations are computed as a streaming operation — the database doesn't need to load all data into memory before computing the mean"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Data is stored row-by-row in timestamp order across all series, and the database uses a bitmap to skip rows from non-matching series"
        },
        {
          "label": "B",
          "text": "Each series' data is stored in its own contiguous block, and the file's index maps series keys to block offsets. The tag index identifies matching series, the file index locates their blocks, and only those blocks are read — two seeks regardless of file size"
        },
        {
          "label": "C",
          "text": "The database maintains a columnar store where all timestamps are in one column and all values in another, enabling column-level filtering"
        },
        {
          "label": "D",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "The WAL is an append-only sequential write — extremely fast, even on spinning disks. It provides crash recovery (replay the WAL to rebuild the memtable) without the random I/O penalty of traditional databases. The memtable provides fast reads and sorted organization, while the WAL provides durability"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Homogeneous data compresses much better — a block of all timestamps achieves better delta-of-delta compression than interleaved timestamp-value pairs. Similarly, a block of all float values achieves better XOR compression. Different data types benefit from different compression algorithms"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "The Bloom filters from the merged SSTables are combined using bitwise OR to create the new SSTable's filter, preserving all entries with zero additional overhead"
        },
        {
          "label": "B",
          "text": "A new Bloom filter must be built from scratch for the merged SSTable. This adds CPU and memory overhead during compaction, but the resulting single filter is more accurate than querying multiple smaller filters because one filter check replaces several"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "1→3→4→2→6→5: WAL first for durability, then memtable (sorted insert), then tag index update. Memtable flushes to SSTable when full. Compaction runs in background. Query checks memtable first for recent data"
        },
        {
          "label": "B",
          "text": "3→1→4→2→5→6: Memtable first for speed, WAL for async durability, then tag index. Flush and compact later"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Layer 1: Time-based partitioning limits to 7 daily partitions (skips older data). Layer 2: Tag index filters to 'env=prod' series only (skips staging/dev blocks). Layer 3: Block-level metadata skips blocks where max < 90% (avoids decompressing blocks that can't contain spikes). Only blocks surviving all three filters are actually read and decompressed"
        },
        {
          "label": "B",
          "text": "Layer 1: Bloom filters check each partition for the 'prod' tag. Layer 2: Binary search within matching partitions finds the relevant time ranges. Layer 3: Block metadata provides the final filter"
        },
        {
          "label": "C",
          "text": "Layer 1: The query planner estimates which partitions have the most data and scans those first. Layer 2: Parallel scans across all partitions. Layer 3: Block-level metadata filters within each parallel scan"
        },
        {
          "label": "D",
          "text": "Layer 1: The inverted tag index provides all matching block offsets directly. Layer 2: Time filtering is applied post-read. Layer 3: Block metadata is checked after decompression"
        }
      ],
      "correct": 0,
