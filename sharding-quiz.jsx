...
const QUESTIONS = [
  {
    ...
    question:
      "Your team is designing a sharding strategy for a social media platform. You're evaluating candidate shard keys: (A) user_id, (B) is_premium (boolean), (C) created_at (timestamp), and (D) country_code. Which would you reject and why?",
    options: [
    ...
    correctIndex: 0,
    explanation:
      "The content states that is_premium is a bad shard key because it has low cardinality — only two values. This causes extremely uneven distribution. created_at is also bad because writes cluster on 'today's date', creating a hot shard that receives all new signups. user_id has high cardinality and distributes evenly, making it a good choice. country_code could work for geographic distribution but has medium cardinality (~250 values).",
    interviewScript:
      "In your interview, say: 'I'd reject is_premium immediately — it's a boolean with only two possible values, so you get 50-50 distribution instead of even load spreading. created_at is equally bad because writes cluster on the current date: all new user signups land on the same shard. user_id is cardinally rich and distributes evenly, making it the right choice here.'",
    proTip:
      "Always evaluate shard keys on cardinality (distinct values) and distribution patterns (are writes/reads clustered in time or by value). Low cardinality is the most common mistake.",
  },
  {
    id: "q2",
    subtopic: "Shard Key Selection",
    ...
    question:
      "You're designing a sharding scheme for an e-commerce platform with customer orders. You choose order_id as the shard key. This distributes orders evenly across shards, but analyst queries consistently ask 'what are total sales by customer?' Which statement best captures the trade-off?",
    options: [
  ...
  },
  {
    ...
    question:
      "You propose user_id as the shard key, claiming it has high cardinality and distributes evenly. An interviewer asks: 'But what if you have a power user platform where 1% of users generate 50% of the data and activity?' What's the strongest response?",
    options: [
  ...
  },
  {
    ...
    question:
      "A candidate proposes hashing the complete user object (serialized JSON) as the shard key for a user service. Why is this a critical anti-pattern?",
    options: [
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "You use hash-based sharding with modulo: shard_id = hash(key) % 4. You're running 4 shards today. In 6 months, you need to add a 5th shard for capacity. You change the formula to shard_id = hash(key) % 5. What happens to your data?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This is the classic hash modulo resharding problem. When you go from % N to % (N+1), the modulo changes for nearly all keys. Only keys where hash(key) % 4 and hash(key) % 5 are the same (roughly 20% of keys) stay on the same shard; 80% need to migrate. The content states this is exactly why 'consistent hashing' was invented — it ensures that adding a node only causes 1/N of keys to migrate, not 4/5 of them.",
    interviewScript:
      "In your interview, say: 'Modulo resharding is expensive. When I go from % 4 to % 5, nearly every key's shard assignment changes. I'd need to migrate billions of keys to their new homes — massive data movement, high operational risk. This is why consistent hashing was invented. With consistent hashing, adding a shard only moves 1/N of keys (20% in this case), not 80%.'",
    proTip:
      "The hash modulo resharding problem is a classic gotcha. Interviewers ask this to see if you understand why consistent hashing matters. Simply saying 'consistent hashing' without explaining the problem is insufficient — explain the % N issue first.",
  },
  {
    id: "q7",
    subtopic: "Sharding Strategies",
    ...
    question:
      "You're sharding a user service and are deciding between hash-based and directory-based sharding. Directory-based means a lookup service stores 'which shard contains which key range.' What is the primary advantage of directory-based sharding and its main downside?",
    options: [
  ...
  },
  {
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You have a 10-shard system using consistent hashing with vnodes (virtual nodes). A new shard is added to the cluster. How many keys must migrate, and why does this matter for operational safety?",
    options: [
    ...
    proTip:
      "Vnodes are a key optimization in modern sharding. Mention them unprompted to show you understand the operational reality of resharding.",
  },
  {
    id: "q9",
    ...
    interviewScript:
      "In your interview, say: 'This is the celebrity problem — certain user_ids are extremely hot. I'd handle it with two layers: (1) dedicated shard for the CEO's data to isolate their load from regular users, and (2) aggressive caching of their profile and tweets in a separate cache tier. Most reads hit the cache and never reach the shard. This way, the CEO's 10K req/sec doesn't contend with regular users.'",
    proTip:
      "The celebrity problem is a classic hot spot scenario. Always mention dedicated shards or compound keys as mitigations. Pure sharding strategy changes won't help — you need targeted solutions.",
  },
  {
    id: "q10",
    subtopic: "Hot Spots & Load Imbalance",
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "You monitor shard load and discover that shard 3 is 5x hotter than others. You increase its replication factor from 3 to 5 and add two read replicas. The next day, shard 3 is still 5x hotter. Why didn't read replication solve the problem?",
    options: [
  ...
  },
  {
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You decide to split the hot shard using a compound key: instead of sharding by user_id, you shard by (user_id, event_type). This spreads a hot user's events across multiple shards. What is the main trade-off introduced by this compound key approach?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Compound keys distribute load better (hot key problem solved) but hurt query alignment (queries by single user now scatter-gather). This is a classic trade-off between load distribution and query efficiency. The content implies these compromises are often necessary in real systems.",
    interviewScript:
      "In your interview, say: 'Compound keys solve the hot shard problem by spreading a user's events across multiple shards. But they introduce a new problem: queries like \"get all events for user X\" now hit multiple shards. It's a trade-off between even load distribution and query efficiency. If the user's hot spot is the critical problem, it's a good trade-off.'",
    proTip: null,
  },
  {
    id: "q13",
    ...
    correctIndex: 1,
    explanation:
      "The content states scatter-gather queries are 'expensive' and 'a sign the design may need rethinking.' A 5-second cross-shard operation on the critical path is problematic. Solutions include precomputation, denormalization, and background jobs — all ways to avoid the scatter-gather at query time.",
    interviewScript:
      "In your interview, say: 'Scatter-gather across 100 shards takes 5 seconds — too slow for a critical path like order placement. This signals a design problem. I'd avoid the cross-shard operation by precomputing the customer's credit status in a separate single-shard index, or denormalizing their current credit into the order table. Then the credit check is just a single-shard lookup. If that's not feasible, I'd batch-update credit limits nightly using a background job.'",
    proTip:
      "Cross-shard operations are a red flag. They signal that either the shard key is misaligned with access patterns, or the query shouldn't be on the critical path.",
  },
  {
    id: "q14",
    subtopic: "Cross-Shard Operations",
    style: "Anti-Pattern Identification",
    question:
      "A candidate proposes caching the result of a scatter-gather query 'sum total orders by user' in Redis with a 1-hour TTL to avoid repeated scatter-gather. The data is used to enforce customer credit limits. Why is this a problematic caching strategy?",
    options: [
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "The content states: use 'background jobs to precompute results' as a solution to expensive cross-shard operations. For analytics workloads with relaxed freshness requirements, batch aggregation is ideal — it moves expensive computation off the critical path and serves pre-computed results.",
    interviewScript:
  ...
  },
  {
    ...
    question:
      "During an interview, the interviewer says: 'Your system requires frequent scatter-gather queries across 50 shards for critical path operations.' What does this signal about your design, and what's your response?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly warns: scatter-gather is 'a sign the design may need rethinking.' If critical operations require hitting all shards, the design is fundamentally misaligned. The answer acknowledges this and outlines legitimate solutions.",
    interviewScript:
      "In your interview, say: 'Frequent scatter-gather on the critical path is a red flag. It signals the shard key is misaligned with how data is actually accessed. I'd reconsider the design: maybe the shard key is wrong, or I need to denormalize critical data to a separate index. Or push this query off the critical path using precomputation or caching. Scatter-gather is expensive and shouldn't be on the critical path if avoidable.'",
    proTip:
      "When interviewer points out scattered-gather, they're testing if you recognize it as a design smell. Acknowledge it frankly and propose solutions.",
  },
  {
    id: "q17",
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly states 2PC is 'slow' and 'fragile.' The blocking problem during network partition (locks held indefinitely) is the key fragility. The content advises 'design to avoid cross-shard transactions' rather than relying on 2PC.",
    interviewScript:
    ...
    proTip:
      "Mention the blocking problem when criticizing 2PC. It's the core reason 2PC fails in distributed systems.",
  },
  {
    id: "q18",
    subtopic: "Maintaining Consistency",
  ...
  },
  {
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You implement a saga pattern for a cross-shard transaction: A debitCustomer, B createOrder, C creditMerchant. Step 2 (createOrder) fails. The saga must compensate: A creditCustomer (undo debit), C debitMerchant (undo credit). What is the key risk of this compensating transaction approach, and how do you mitigate it?",
    options: [
    ...
    correctIndex: 0,
    explanation:
      "The content mentions saga pattern as an alternative to 2PC. The key insight is that sagas trade consistency for availability — they're loosely consistent. The compensating transactions themselves can fail, requiring idempotency and durability.",
    interviewScript:
      "In your interview, say: 'The compensation itself could fail. If creditCustomer fails, the customer is permanently debited. The mitigation is to make all compensations idempotent and durable. Before executing a compensation, log it durably so it can be retried if it fails. And make creditCustomer idempotent — it's safe to retry if the first attempt partially succeeded.'",
    proTip:
      "Sagas are not a 'easy alternative' to 2PC. They're loosely consistent and require careful handling of compensating transaction failures.",
  },
  {
    id: "q20",
    subtopic: "Maintaining Consistency",
    style: "Gotcha/Trap",
    question:
      "Your team claims that eventual consistency is 'fine for any system' because 'data is eventually correct.' They propose using eventual consistency for a payment system where customers make purchases and reconcile accounting with the payment provider. What's wrong with this reasoning?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly names scenarios where eventual consistency is acceptable (analytics, metrics) and implies financial transactions are not. The window of inconsistency in a payment system creates liability and user confusion.",
    interviewScript:
      "In your interview, say: 'Eventual consistency means there's a window where the system and payment provider disagree on the transaction state. A customer sees their account debited but the order isn't created. That window is unacceptable for a payment system. The content is clear: eventual consistency is fine for analytics and metrics, but not for transactions involving money. For payments, I need strong consistency or extremely tight windows with compensating mechanisms if inconsistency is detected.'",
    proTip:
      "This is a fundamental concept: consistency models are workload-dependent. What's fine for metrics is unacceptable for money.",
  },
  {
    id: "q21",
    ...
    style: "Anti-Pattern Identification",
    question:
      "A junior engineer says: 'Sharding is the same as partitioning. Both split data across storage.' Why is this a misleading conflation, and what's the key difference?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content states: 'partitioning within a single database (horizontal/vertical), sharding across machines. The terms are used loosely.' This distinction is critical for design decisions.",
    interviewScript:
      "In your interview, say: 'Partitioning and sharding sound similar but are different. Partitioning splits data within a single database — still one machine, one coordinator. Sharding splits across multiple machines — fully distributed. Confusing them is dangerous. If you think you only need partitioning and sharding becomes necessary later, you've already made architectural decisions that don't scale. The distinction matters.'",
    proTip: null,
  },
  {
    id: "q22",
    ...
    style: "Decision Framework Application",
    question:
      "You're designing a system with 10GB of data today and expecting 100GB in a year. Your current database is PostgreSQL on a single 2TB server. Should you shard now, or start with partitioning and shard later if needed?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content emphasizes avoiding premature sharding. If the data fits on a single machine with one partition, start there. Sharding should come only when you exceed single-machine capacity or hit other bottlenecks (write throughput, connection limits). 100GB is easily within a 2TB machine's capacity.",
    interviewScript:
      "In your interview, say: 'With 100GB of projected data, a single 2TB server handles it fine. I'd start with PostgreSQL partitioning to split the data logically. Only if I hit write throughput limits, connection exhaustion, or exceed the machine's capacity would I shard across multiple machines. The content emphasizes avoiding premature sharding — it adds distributed complexity that's not justified by 100GB of data.'",
    proTip:
      "Premature sharding is a cardinal sin. Always start simple and shard only when necessary.",
  },
  {
    id: "q23",
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You're choosing a database for a new sharded service. Cassandra uses Murmur3 hash with vnodes, DynamoDB uses internal partitions, MongoDB uses range-based chunks with hashed shard key. Which is the right choice for a multi-tenant analytics platform where tenants are isolated and queries are often tenant-specific?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content mentions that modern databases use different sharding strategies. MongoDB's approach (hashed shard key, automatic balancing) is well-suited for multi-tenant systems where even distribution is important. The automatic balancer solves the operational complexity of manual chunk migration.",
    interviewScript:
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "This highlights that transparent sharding hides complexity but doesn't eliminate it. Even with automated routing, distributed joins are expensive. Vitess makes it easier but doesn't change the fundamental cost.",
    interviewScript:
      "In your interview, say: 'Transparent routing is convenient, but it doesn't eliminate the cost of distributed joins. Even though both tables are sharded by user_id, the join still requires correlating rows across shards for each user. For large datasets, that's expensive. I'd optimize by denormalizing order.total into user, or recomputing the join offline into a separate materialized view. Transparency doesn't mean free.'",
    proTip:
      "Transparent sharding tools (Vitess, Citus) are powerful but can hide distributed complexity. Know the costs even when routing is automatic.",
  },
  {
    id: "q25",
    ...
    style: "Anti-Pattern Identification",
    question:
      "An interviewer asks you to design a social media feed service. A candidate immediately says: 'I'll shard by user_id across 10 shards to handle the scale.' The interviewer asks: 'What scale?'. The candidate realizes the system needs to support 1M users, which fits on a single large machine. What's the mistake here?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content emphasizes 'when to mention sharding' and warns against premature sharding. The interview strategy is to identify the bottleneck first, then propose sharding as a solution to that specific bottleneck.",
    interviewScript:
      "In your interview, say: 'With 1M users, a single machine easily handles the data and throughput. Proposing sharding without identifying a bottleneck is premature sharding. The right approach is to start simple: estimate the database size (probably 10-50GB for 1M users), estimate throughput (reads/sec, writes/sec), and check if a single machine can handle it. Only if we hit the machine's limits do we shard. This shows I understand the tradeoffs.'",
    proTip:
      "Premature sharding is the most common mistake in interviews. Always start simple and shard only when justified by a real bottleneck.",
  },
  {
    id: "q26",
    subtopic: "Interview Strategy",
    style: "Decision Framework Application",
    question:
      "You're designing a system that needs to support 100 million users and 10 billion events per day. An interviewer asks: 'How would you introduce sharding into your design walk-through?' What's the right approach?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content states the interview strategy is to: 'identify the bottleneck (storage/write/read), then propose sharding as a solution, walk through shard key and strategy, address trade-offs.' Option B follows this exactly.",
    interviewScript:
      "In your interview, say: 'Let me estimate the bottleneck. 10B events/day is about 115K writes/sec. A single database instance with replication can handle ~50K write throughput. That's insufficient, so I need sharding to distribute writes. I'd shard by event_id or user_id depending on access patterns. Hash-based sharding distributes evenly. The trade-off is that cross-shard queries become expensive, which is acceptable if the primary path is writing events.'",
    proTip:
      "Always show your bottleneck analysis before proposing sharding. This demonstrates deep understanding, not pattern matching.",
  },
  {
    id: "q27",
    ...
    style: "Critical Failure Modes",
    question:
      "Explain the difference between horizontal and vertical partitioning. When would you use each?",
    options: [
  ...
  },
  {
    ...
    style: "Estimation-Backed Reasoning",
    question:
      "Explain consistent hashing and why it's better than modulo-based hashing for sharded systems.",
    options: [
    ...
    correctIndex: 0,
    explanation:
      "The content explicitly mentions consistent hashing as a solution to the hash modulo resharding problem. The 1/N key migration advantage is key.",
    interviewScript:
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "You implement hash-based sharding with modulo: shard = hash(key) % 4. You have 4 shards today. You want to add a 5th shard. Using consistent hashing, what percentage of keys need to migrate?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Consistent hashing's advantage is directly quantifiable: adding a node moves 1/N of keys, not (N-1)/N. This is the math that makes consistent hashing operationally superior.",
    interviewScript:
      "In your interview, say: 'With modulo % 4 to % 5, 80% of keys change shard assignment — massive resharding. Consistent hashing reduces this to about 20%, or 1/(N+1). That's the operational advantage: adding a node only migrates 1/5 of the keyspace, not 4/5.'",
    proTip:
      "The percentages matter. Know that consistent hashing reduces resharding from (N-1)/N to 1/N.",
  },
  {
    id: "q30",
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You're rebalancing a sharded system because one shard is 10x hotter than others. You need to split the hot shard into two shards and migrate half the data. During migration, the hot shard serves both the old and new ranges, causing write latency to spike. How would you minimize disruption to clients?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Real systems implement sophisticated migration strategies to minimize disruption. The most common approaches involve double-write periods (where writes go to both old and new shards), versioned shard assignments, and eventual consistency.",
    interviewScript:
      "In your interview, say: 'Migrations cause disruption, but I can minimize it with careful engineering. Use a versioned shard assignment that clients check on each write — if the version has changed, write to both old and new shards briefly. Or implement eventual consistency: allow writes to land on the old shard, then migrate them to the new shard in the background. This spreads the data movement over time instead of causing a single latency spike.'",
    proTip:
      "Rebalancing is a practical problem with real operational solutions. Mention these to show production experience.",
  },
  {
    id: "q31",
    ...
    style: "Critical Failure Modes",
    question:
      "What are virtual nodes (vnodes) in sharding, and why do modern systems like Cassandra use them instead of direct node assignment?",
    options: [
  ...
  },
  {
    ...
    style: "Failure Analysis",
    question:
      "Your sharded e-commerce system is sharded by user_id. You need to support queries like 'find all orders with status=pending across all users.' You create a secondary index on status field. But queries still require scatter-gather across all shards. Why doesn't the secondary index help?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Secondary indexes in sharded systems are local per shard, not global. This is a subtle but important limitation that affects query patterns.",
    interviewScript:
      "In your interview, say: 'Each shard has its own status index, but it only covers that shard's data. A global query for pending orders must still hit all shards. A true global secondary index would help, but it's complex to maintain and becomes a bottleneck. For this query pattern, I'd use a separate index sharded by status, or denormalize status into the primary shard key alongside user_id.'",
    proTip:
      "Understanding the limitations of secondary indexes in sharded systems shows operational depth.",
  },
  {
    id: "q33",
    ...
    style: "Critical Failure Modes",
    question:
      "A platform shards by region (e.g., US, EU, APAC). Each region shard handles millions of users. But suddenly, the US shard becomes 5x hotter than others due to a viral event. The shard key (region) is now misaligned with actual load. How would you address this without changing the shard key?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This bridges shard key selection and hot spots. Even a reasonable shard key (region) can create hot spots if the underlying workload is skewed. Short-term mitigations exist, but the long-term solution might be rethinking the shard key.",
    interviewScript:
      "In your interview, say: 'Region is a reasonable shard key until workload becomes skewed. The viral event exposed its limitation. Short-term: split the US shard into sub-regions, replicate to spread read load, or cache the hot data. But this is a symptom that region isn't the right key for this workload's patterns. Long-term, I'd redesign to shard by user_id with region as a secondary attribute.'",
    proTip: null,
  },
  {
    id: "q34",
    ...
    style: "Decision Framework Application",
    question:
      "You're evaluating between MongoDB (sharded by hashed shard key) and DynamoDB (sharded internally by partition key) for a real-time recommendation service. Both support sharding. Which is a better fit and why?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content mentions that modern databases shard differently. The choice depends on whether you need control and complexity (MongoDB) or simplicity and auto-scaling (DynamoDB). For recommendation services with complex queries, MongoDB's transparency matters.",
    interviewScript:
  ...
  },
  {
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You have a user service sharded by user_id and an order service sharded by user_id. A user deletes their account, which requires: (1) delete user from user service, (2) delete orders from order service, (3) send deletion notification. These happen across two shards. How would you implement this safely?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This applies saga pattern to a real-world scenario. Event-driven sagas are the modern solution to cross-shard consistency.",
    interviewScript:
      "In your interview, say: 'I'd use a saga pattern with events. User deletion triggers a 'deleteUser' event. The user service processes it and deletes, emitting 'userDeleted'. The order service processes and deletes, emitting 'ordersDeleted'. The notification service sends the notification. If order deletion fails, a compensating transaction re-creates the user (if that's safe). This avoids 2PC's blocking and gives me eventual consistency for account deletion.'",
    proTip: null,
  },
  {
    id: "q36",
    ...
    style: "Interviewer Pushback",
    question:
      "In an interview, you propose a design for a 10M user system. The interviewer says: 'I'm concerned you're over-engineering. Is sharding really necessary here?' What's the best response that shows you understand trade-offs?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This demonstrates the interview strategy the content recommends: start simple, identify the bottleneck, introduce sharding as a solution.",
    interviewScript:
      "In your interview, say: 'Good point. Let me estimate: 10M users with 1KB metadata is 10GB — single machine handles that. For throughput, without knowing the QPS, I'll assume a typical system: maybe 100-200 QPS reads, easily handled by one database. Sharding adds complexity for resharding, routing, distributed ops. Without a clear bottleneck, it's premature. I'd start simple, monitor, and shard only if we hit capacity or throughput limits.'",
    proTip:
      "Being willing to simplify your design when challenged shows confidence and understanding. Interviewers respect this more than blind insistence on sharding.",
  },
];

const SUBTOPIC_TIERS = {
  Heavy: [
    { name: "Shard Key Selection", count: 4 },
    { name: "Sharding Strategies", count: 4 },
    { name: "Hot Spots & Load Imbalance", count: 4 },
    { name: "Cross-Shard Operations", count: 4 },
    { name: "Maintaining Consistency", count: 4 },
  ],
  Medium: [
    { name: "Partitioning vs Sharding", count: 2 },
    { name: "Modern Database Sharding", count: 2 },
    { name: "Interview Strategy", count: 2 },
  ],
  Thin: [
    { name: "Horizontal vs Vertical Partitioning", count: 1 },
    { name: "Consistent Hashing", count: 1 },
    { name: "Hash Modulo Resharding Problem", count: 1 },
    { name: "Shard Rebalancing", count: 1 },
    { name: "Virtual Nodes / Vnodes", count: 1 },
    { name: "Secondary Indexes in Sharded Systems", count: 1 },
    { name: "Shard Key Selection × Hot Spots", count: 1 },
    { name: "Sharding Strategies × Modern DBs", count: 1 },
    { name: "Cross-Shard Ops × Consistency", count: 1 },
  ],
};

export default function ShardingQuiz({ quizSlug }) {
  ...
  const handleResume = () => {
    ...
    const restoredAnswers = resumeData.questionResults || {};
    setAnswers(restoredAnswers);
    setFlagged(new Set());
    
    if (resumeData.questionOrder && resumeData.questionOrder.length === QUESTIONS.length) {
      const restored = resumeData.questionOrder.map(id => QUESTIONS.find(q => q.id === id)).filter(Boolean);
    ...
    setCurrentIndex(resumeData.answeredCount || 0);
    setScreen("question");
    resumeAttempt();
  };

  useEffect(() => {
    if (screen !== "question" || isTimeUp) return;
    setTimeRemaining(90);
    setIsTimeUp(false);
  ...
  const handleAutoSubmit = useCallback(() => {
    if (selected === null) {
      ...
      const newAnswers = {
        ...answers,
        [orderedQuestions[currentIndex].id]: answerObj,
      };
      setAnswers(newAnswers);
      saveAnswer(orderedQuestions[currentIndex].id, answerObj);
      handleNext();
    }
  }, [selected, answers, orderedQuestions, currentIndex, confidence, saveAnswer]);

  const handleSelectOption = (index) => {
    if (!isTimeUp) setSelected(index);
  };
  ...
  const handleSubmit = () => {
    ...
    setAnswers(newAnswers);
    saveAnswer(question.id, answerObj);
    setAnswerSheet({
      question,
      isCorrect,
      selected,
  ...
  const handleCompleteQuiz = () => {
    ...
    completeQuiz({ score: finalScore, totalTime: 900 });
    setScreen("results");
  };
  ...
  const handleShuffle = () => {
    ...
    setConfidence(null);
    setFlagged(new Set());
    startNewAttempt(shuffled.map(q => q.id));
    setScreen("question");
  };
  ...
  const handleRetry = (questionsToRetry) => {
    const retryQuestions = orderedQuestions.filter((q) =>
      questionsToRetry.some((rq) => rq.id === q.id)
    );
    setOrderedQuestions(retryQuestions);
    setCurrentIndex(0);
    setAnswers({});
    setSelected(null);
    setConfidence(null);
  ...
  // Keyboard shortcuts
  useEffect(() => {
    if (screen !== "question" || answerSheet) return;
    const handleKeyPress = (e) => {
      if (["1", "2", "3", "4"].includes(e.key)) {
        handleSelectOption(parseInt(e.key) - 1);
      } else if (["a", "b", "c", "d"].includes(e.key.toLowerCase())) {
        handleSelectOption(e.key.toLowerCase().charCodeAt(0) - "a".charCodeAt(0));
      } else if (e.key === "Enter") {
        if (selected !== null) handleSubmit();
      }
    };
    window.addEventListener("keydown", handleKeyPress);
    return () => window.removeEventListener("keydown", handleKeyPress);
  }, [screen, selected, answerSheet, handleSelectOption, handleSubmit]);

  if (screen === "landing") return <LandingScreen onStart={handleStartQuiz} onShuffle={handleShuffle} isResuming={isResuming} resumeData={resumeData} onResume={handleResume} />;
  if (screen === "question")
    return (
      <QuestionScreen
        ...
        onSkip={handleSkip}
        onFlag={handleFlag}
        isFlagged={flagged.has(orderedQuestions[currentIndex].id)}
...
function LandingScreen({ onStart, onShuffle, isResuming, resumeData, onResume }) {
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4 flex items-center justify-center">
      <div className="max-w-2xl w-full">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold mb-3">Sharding</h1>
          <p className="text-gray-400 text-lg">
            Master distributed data partitioning for system design interviews
          </p>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-8 mb-6">
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-red-400">FAANG SDE2</div>
          ...
          <div className="border-t border-gray-800 pt-6">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <BarChart3 size={18} /> Subtopic Breakdown
            </h3>
            {["Heavy", "Medium", "Thin"].map((tier) => (
              <div key={tier} className="mb-4 last:mb-0">
                ...
                >
                  {tier} — {SUBTOPIC_TIERS[tier].reduce((sum, t) => sum + t.count, 0)} questions
                </div>
                <div className="text-sm text-gray-400">
                  {SUBTOPIC_TIERS[tier].map((t) => t.name).join(", ")}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={onStart}
            className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-blue-600 hover:bg-blue-700 rounded-lg font-semibold transition-colors"
          >
            <List size={18} /> Start Quiz (Section Order)
          </button>
          <button
            onClick={onShuffle}
            className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-gray-800 hover:bg-gray-700 rounded-lg font-semibold transition-colors"
          >
            <Shuffle size={18} /> Start Quiz (Shuffled)
          </button>
          {isResuming && resumeData && (
            <button
              onClick={onResume}
              className="flex items-center justify-center gap-2 w-full px-6 py-4 bg-teal-600 hover:bg-teal-700 rounded-lg font-semibold transition-colors"
            >
              <RotateCcw size={18} /> Resume Quiz ({resumeData.answeredCount}/{QUESTIONS.length})
            </button>
          )}
        </div>
      </div>
...
function QuestionScreen({
  ...
  const isCorrect = answerSheet?.isCorrect;
  const showFeedback = answerSheet !== null;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Subtopic tag */}
        <div className="mb-4">
          <span className="text-xs px-3 py-1 bg-indigo-900/40 border border-indigo-700/40 text-indigo-300 rounded">
            {question.subtopic}
          </span>
        </div>
              ...
              <div className="flex items-start gap-3">
                <div
                  className={`flex items-center justify-center w-6 h-6 rounded border-2 flex-shrink-0 mt-0.5 ${
                    showFeedback
                      ? index === question.correctIndex
                        ? "border-green-500 bg-green-500 text-gray-950 font-bold"
                        : index === answerSheet.selected
                        ? "border-red-500 bg-red-500 text-gray-950 font-bold"
                        : "border-gray-600 text-gray-600"
                      : selected === index
                      ? "border-blue-500 bg-blue-500 text-gray-950 font-bold"
                      : "border-gray-600 text-gray-600"
                  }`}
                >
        ...
        {showFeedback && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className={isCorrect ? "text-green-400" : "text-red-400"} style={{fontWeight: 600}}>
            ...
            {question.interviewScript && (
              <div className="bg-gray-800 rounded-lg p-4 border-l-2 border-blue-500">
        ...
        <div className="flex gap-3 justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg font-semibold transition-colors"
          >
            {showFeedback ? (
              <>
...
function ResultsScreen({
  ...
  const grade =
    score >= 90
      ? { label: "Excellent", bg: "bg-green-900/30", color: "text-green-400" }
      : score >= 75
      ? { label: "Good", bg: "bg-blue-900/30", color: "text-blue-400" }
      : score >= 60
      ? { label: "Fair", bg: "bg-amber-900/30", color: "text-amber-400" }
      : { label: "Needs Improvement", bg: "bg-red-900/30", color: "text-red-400" };

  const incorrect = orderedQuestions.filter(
    (q) => answers[q.id] && !answers[q.id].correct
  );
  const flaggedQs = orderedQuestions.filter((q) => flagged.has(q.id));
  const luckyGuesses = orderedQuestions.filter(
    (q) =>
      answers[q.id] &&
      answers[q.id].correct &&
      answers[q.id].confidence === "Guessing"
  );
  const overconfidentMisses = orderedQuestions.filter(
    (q) =>
      answers[q.id] &&
      !answers[q.id].correct &&
      answers[q.id].confidence === "Very Confident"
  );
  const weakQuestions = [...luckyGuesses, ...overconfidentMisses];

  const subtopicStats = {};
  orderedQuestions.forEach((q) => {
    if (!subtopicStats[q.subtopic]) {
      subtopicStats[q.subtopic] = { correct: 0, total: 0 };
    }
  ...
  const totalElapsed = Object.values(answers).reduce((sum, a) => {
    return sum + (a.timestamp || 0);
  }, 0);

  const getTier = (name) => {
    for (const [tier, topics] of Object.entries(SUBTOPIC_TIERS)) {
      if (topics.some((t) => t.name === name)) return tier;
    }
    return "Thin";
  };
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <div
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border ${grade.bg} ${grade.color}`}
          >
            <Award size={18} />
            {grade.label}
          </div>
                      ...
                      <div className="w-full bg-gray-800 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full transition-all ${
                            pct >= 70 ? "bg-green-500" : "bg-red-500"
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
            ...
            {luckyGuesses.length > 0 && (
              <div className="mb-4">
                <div className="text-sm font-medium text-amber-400 mb-2">
                  Lucky Guesses — correct but you were guessing (hidden weak
                  spots)
                </div>
                {luckyGuesses.map((q) => (
                  <div
                  ...
                  >
                    <span className="text-amber-500">{q.id}</span> —{" "}
            ...
            {overconfidentMisses.length > 0 && (
              <div>
                <div className="text-sm font-medium text-red-400 mb-2">
                  Overconfident Misses — wrong but you were very confident
                ...
                {overconfidentMisses.map((q) => (
                  <div
                    ...
                    className="text-sm text-gray-400 py-1 border-b border-gray-800 last:border-0"
                  >
                    <span className="text-red-500">{q.id}</span> —{" "}
                    {q.subtopic}: {q.question.slice(0, 80)}...
                  </div>
                ))}
              </div>
        ...
        {incorrect.length > 0 && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <XCircle size={18} className="text-red-400" /> Incorrect
              Questions ({incorrect.length})
            </h3>
        ...
        {flaggedQs.length > 0 && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <Flag size={18} className="text-amber-400" /> Flagged for Review
              ({flaggedQs.length})
            </h3>
            <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
              {flaggedQs.map((q) => (
