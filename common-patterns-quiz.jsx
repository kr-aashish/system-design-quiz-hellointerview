...
const QUESTIONS = [
  ...
  // ============================================================
  {
    ...
    correctIndex: 1,
    explanation:
      "Pattern recognition is a senior-vs-junior signal precisely because it lets the engineer pre-load the *interesting decision points* and *known failure modes* of a problem. X is doing component recall; Y is doing pattern recognition. In a 45-minute interview, the time savings of skipping reverse-engineering and going straight to the trade-offs is the difference between finishing a solid design and running out of clock.",
    interviewScript:
      "In your interview, say: 'Before I list components, this looks like a realtime-updates problem. There are two decisions to make: client↔server protocol and how updates propagate across servers to the right connection. Each has known trade-offs — let me walk through them and pick.'",
    proTip:
      "When a problem matches a pattern you know, *say so out loud* and then enumerate the interesting decisions the pattern surfaces. This explicit framing is a strong senior signal.",
  },
  {
    id: "qA2",
    part: "A — Why Patterns Matter",
    ...
    l5Pattern: null,
    question:
      "Mid-interview the interviewer says: 'You've named load balancers, caches, queues, and a sharded DB. But what *pattern* are you applying here?' What is the interviewer probing — and how is a pattern different from the components a candidate has just listed?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Patterns are *built from* key technologies and core concepts, but they aren't the same thing. A pattern is the recurring problem shape and the trade-offs that come with solving it — it tells you which components to combine, in what order, and what the failure modes are. The interviewer's prompt is asking the candidate to step from atoms to recipe.",
    interviewScript:
      "In your interview, say: 'Right — these components are the atoms. The pattern I'm applying is [X], which says: when you face [Y problem shape], combine these components this way because [trade-off]. Naming the pattern lets us focus on the interesting decisions instead of re-deriving them.'",
    proTip:
      "Patterns sit one layer above components. When asked 'what pattern,' name the recurring problem shape and the trade-off the pattern encodes — not just a different label for the same set of boxes.",
  },
  {
    id: "qA3",
    part: "A — Why Patterns Matter",
    ...
    correctIndex: 1,
    explanation:
      "Patterns let you skip reverse-engineering common failure modes. A's first-principles approach is fine when truly novel, but at 25 minutes in, the interviewer is testing whether the candidate can *anticipate* failure modes from a recognizable shape. B has compressed minutes of derivation into seconds and bought back interview time for the trade-off discussion the interviewer actually wants. Pattern recognition isn't a substitute for understanding — it's what understanding looks like once it's been compiled.",
    interviewScript:
      "In your interview, say: 'I recognize this as a cold-cache stampede — the failure mode is a thundering herd on the DB. Standard mitigations are request coalescing in the cache layer and jitter on cache fills. The trade-off is added cache complexity vs DB protection during launches and partial outages.'",
    proTip:
      "When you spot a known failure mode, name it explicitly. Naming it shows you've internalized the pattern; deriving it from first principles every time is what a junior does.",
  },
  {
    id: "qA4",
    part: "A — Why Patterns Matter",
    ...
    l5Pattern: null,
    question:
      "A candidate says: 'I'd add a queue here for resilience.' The interviewer pushes: 'Those are tools, not a decision — why is *this* the right pattern for the problem in front of us, and what specifically does it cost?' Two candidates respond: X confidently lists three patterns they could use. Y says: 'The job is sub-second and synchronous from the user's view, so a queue costs us clearer back-pressure and a worse UX in exchange for fault isolation we don't need yet.' What weakness is the interviewer exposing in candidate X?",
    options: [
  ...
  // ============================================================
  {
    ...
    l5Pattern: null,
    question:
      "Two surfaces in the same app: (X) the user pulls down to refresh their bank balance and the new number appears; (Y) the user sees a 'new message' toast within 200ms of a friend sending it, without doing anything. Which surface needs the realtime-updates pattern, and why doesn't the other one?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "Realtime push is for situations where the *update* originates somewhere other than the user's own action — chat, notifications, live dashboards. Synchronous request/response handles cases where the user's action is the trigger; the response *is* the update. Recognizing which surfaces need push and which don't is the first decision in this pattern.",
    interviewScript:
      "In your interview, say: 'For a feature like X, request/response is enough — the response carries the update. For Y, the update originates on a different client or system, so the server has to initiate the push. That's where realtime updates becomes a real pattern.'",
    proTip:
      "Before reaching for SSE/WebSockets, check whether the update has a client-side trigger. If yes, you might not need realtime at all.",
  },
  {
    id: "qB2",
    part: "B — Pushing Realtime Updates",
    ...
    l5Pattern: null,
    question:
      "A candidate says: 'I'd use WebSockets for live updates.' The interviewer asks: 'And how does an update from User A's server reach the WebSocket connection that User B is holding open on a different server?' Why is this question not redundant with the original answer?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "Realtime updates is a two-axis decision: (1) client↔server protocol (polling, SSE, WebSockets), and (2) cross-server propagation (Pub/Sub, consistent-hash routing, etc.). A candidate who answers only the first axis has answered half the question. The interviewer is probing whether the candidate sees both.",
    interviewScript:
      "In your interview, say: 'There are two decisions: how the client and server stay connected — protocol — and how an update reaches the server holding the right connection — propagation. WebSockets is one answer to the first; the second is independent. For this workload I'd pick X for protocol and Y for propagation, because…'",
    proTip:
      "Always present realtime updates as two independent decisions. Candidates who collapse them into one consistently miss the trickier half.",
  },
  {
    id: "qB3",
    part: "B — Pushing Realtime Updates",
    ...
    l5Pattern: null,
    question:
      "A team is building a stock ticker for a brokerage. PMs say 'real-time prices.' The candidate's first instinct is WebSockets. What's the more senior first move and why?",
    options: [
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "The realtime-updates pattern explicitly notes that the infrastructure for SSE and WebSockets is 'tricky to get right.' That trickiness — sticky sessions, connection-state tracking, reconnection, half-open detection — is the cost candidates routinely under-weight. B is not being timid; B is articulating the trade-off A is treating as zero.",
    interviewScript:
      "In your interview, say: 'WebSockets has the latency advantage, but the cost is operational: sticky sessions or distributed connection state, idle-timeout handling, and client-side reconnection logic. Before picking it I want to know whether the latency requirement actually needs sub-second freshness, because if 2-5s works, polling skips all of that.'",
    proTip:
      "Operational complexity is a first-class trade-off. If a candidate picks WebSockets without articulating the operational cost, the interviewer will probe — even if WebSockets is the correct final answer.",
  },
  {
    id: "qB4",
    part: "B — Pushing Realtime Updates",
    ...
    l5Pattern: null,
    question:
      "For a real-time multiplayer drawing app where 50 users in a room share a canvas with sub-100ms cursor and stroke updates, a candidate proposes Pub/Sub for cross-server message delivery. The interviewer says, 'I see why that's tempting — what's the better choice for THIS workload, and why?'",
    options: [
  ...
  },
  {
    ...
    question:
      "A team builds a live dashboard with SSE. Latency tests in dev pass perfectly; in prod, users see updates arrive in 30-second batches. The team blames SSE and starts a migration to WebSockets. Why is the protocol choice the wrong layer to investigate, and what is the actual root cause likely to be?",
    options: [
  ...
  // ============================================================
  {
    ...
    l5Pattern: null,
    question:
      "A user uploads a 2-minute video to a sharing app. The server transcodes it to 5 resolutions — about 90 seconds of compute. The mobile app shouldn't sit on a spinner for 90 seconds. Which response shape is correct, and what infrastructure does it imply?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This is the canonical shape of the long-running-tasks pattern: split the request into the synchronous part (validate, enqueue, return job ID) and the asynchronous part (worker consumes queue, executes work, updates status). The infrastructure is a message queue plus a worker pool — both explicitly named in the pattern.",
    interviewScript:
      "In your interview, say: 'The web server validates the upload, enqueues a transcode job, returns a job ID in milliseconds. A worker pool — separate process — consumes the queue and does the 90s of work. The client polls the job ID or subscribes for completion. The web server's request budget stays in the millisecond range; long work scales independently on the worker side.'",
    proTip:
      "Whenever a piece of work exceeds a few seconds, the default shape is ack-and-defer. The hard part is *not* applying it (see the L5 question for this Part).",
  },
  {
    id: "qC2",
    part: "C — Managing Long-Running Tasks",
    ...
    difficulty: "L2",
    style: "Multi-Hop Reasoning",
    conceptIds: ["C2", "C3", "C4", "C9"],
    l5Pattern: null,
    question:
      "A candidate says, 'I'd push the job to a queue and a worker will pick it up.' The interviewer asks: 'Walk me through what the *web server* does and what the *worker* does — and why that separation matters.'",
    options: [
  ...
  },
  {
    ...
    l5Pattern: null,
    question:
      "A candidate proposes a queue + worker pool for a 'send password reset email' feature. The interviewer pushes back: 'Is that the right pattern here?' How should the candidate answer?",
    options: [
  ...
  },
  {
    ...
    l5Pattern: null,
    question:
      "An e-commerce site sees web traffic 10× during a flash sale. Their architecture has the web servers and the email-sending workers in the same auto-scaling group. Why is this an anti-pattern relative to what the long-running-tasks pattern is trying to give you?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Independent scaling of web servers vs workers is one of the three explicitly-named benefits of the pattern. Bundling them defeats it. Web servers scale with user concurrency; workers scale with queue depth. Different load shapes need different scaling curves; sharing a group means you optimize for neither.",
    interviewScript:
      "In your interview, say: 'The point of separating web from worker is that they have different load shapes. Web is bursty — driven by user concurrency. Workers are throughput-bound — driven by queue depth. Co-scaling means I either over-provision workers during traffic spikes or starve the queue during quiet periods. They should be separate auto-scaling groups with separate metrics.'",
    proTip:
      "When you call out independent scaling, name the metric each side scales on. Web: concurrent connections / RPS. Worker: queue depth / oldest message age. Different metrics ⇒ different groups.",
  },
  {
    id: "qC4",
    part: "C — Managing Long-Running Tasks",
    ...
    l5Pattern: null,
    question:
      "A candidate's design has a worker pool consuming a Kafka topic. The interviewer asks: 'What happens if a single message has bad data that crashes the worker every time it tries to process it?'",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "Poison messages are one of the explicit failure scenarios the pattern names. Without bounded retries and a DLQ, a single bad message becomes a denial-of-service against the worker pool. The pattern's required handling list — 'job status tracking, retries, and failure scenarios like dead-letter queues for poison messages' — is exactly this. Idempotency is the companion requirement: any retry-based design must assume duplicate processing.",
    interviewScript:
      "In your interview, say: 'Without protection, a single poison message stalls the partition. I'd add bounded retries with exponential backoff — say three attempts — and on final failure move the message to a DLQ for human triage. The worker logic also has to be idempotent, because retries mean the same job may execute more than once.'",
    proTip:
      "Whenever you draw a queue + worker, draw the DLQ next to it. It's one box, but its absence is a fast way to lose senior signal.",
  },
  {
    id: "qC5",
    part: "C — Managing Long-Running Tasks",
  ...
  // ============================================================
  {
    ...
    l5Pattern: null,
    question:
      "On Black Friday, a single product has 5 units left in inventory. Within 100 milliseconds, 50,000 users hit 'Buy Now.' The naive design reads 'available > 0' and decrements. Without coordination, what goes wrong, and what general class of problem is this?",
    options: [
  ...
  },
  {
    ...
    l5Pattern: null,
    question:
      "For a global cart-checkout flow, you can use 2PC across the cart-service DB and inventory-service DB to keep them perfectly consistent, OR a saga (cart-service writes, then asks inventory-service to reserve, then compensates if anything fails). For a high-throughput consumer site, which fits, and why?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The pattern names the trade-off explicitly: 'performance versus consistency guarantees, and simple database solutions versus complex distributed coordination.' 2PC gives strong consistency at the cost of throughput and availability — the coordinator becomes a synchronous dependency for every transaction and a single point of failure. Saga gives eventual consistency with much higher throughput and resilience. For a consumer checkout that can tolerate brief inconsistency (with compensations), the saga is almost always the senior choice. The deeper move is sometimes recognizing that the data-split itself was premature (see Part D L5).",
    interviewScript:
      "In your interview, say: 'I'd use a saga. 2PC across two services adds a synchronous coordinator that's now in the critical path for every checkout — that's a throughput ceiling and a single point of failure. Saga accepts eventual consistency, with explicit compensations on failure, in exchange for the throughput and availability we actually need at scale.'",
    proTip:
      "Whenever you compare 2PC vs saga, frame it as 'consistency vs availability/throughput.' The choice is workload-dependent — but the framing itself is what shows you understand the pattern.",
  },
  {
    id: "qD3a",
    part: "D — Dealing with Contention",
    ...
    l5Pattern: null,
    question:
      "A candidate designs an event-bookings system with separate user-service, booking-service, and inventory-service, each with its own database. The interviewer asks: 'What changes when a customer cancels and you have to issue a refund?' Why is this question a probe rather than a feature request?",
    options: [
  ...
  },
  {
    ...
    l5Pattern: null,
    question:
      "A candidate immediately designs a sharded inventory service with a Redis-based distributed lock for a startup that processes 100 orders per day. The interviewer asks: 'Are we sure this is the right shape?' What's the issue?",
    options: [
  ...
  },
  {
    ...
    question:
      "A candidate proposes: 'To scale write throughput, I'll shard the orders table by user_id.' The interviewer says: 'Couldn't you just do that? It seems clean.' What subtle problem does sharding *introduce* that the original single-database design *solved for free*?",
    options: [
  ...
  // ============================================================
  {
    ...
    l5Pattern: null,
    question:
      "A new social app has 10,000 users; the database is fine. The interviewer scales the brief to 10 million users and asks: 'Where do you expect the first scaling problem to appear — reads, writes, or storage?' What's the right answer and why?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "The pattern's framing line: 'As your application grows from hundreds to millions of users, read traffic often becomes the first bottleneck. While writes create data, reads consume it — and read traffic typically grows much faster than write traffic.' Recognizing this asymmetry is the entry point to the entire pattern.",
    interviewScript:
      "In your interview, say: 'Reads will hit limits first. Read traffic grows with user count *and* per-user read frequency — each feed view fires many queries — while write rate per user grows slowly. The asymmetry is usually 10:1 to 100:1, so the strategies are asymmetric too: read replicas, caches, indexing.'",
    proTip:
      "When the interviewer scales up the brief, default-assume reads bottleneck first. The exceptions (analytics ingest, IoT telemetry) are the cases where you should call out 'this is unusual — writes dominate' explicitly.",
  },
  {
    id: "qE2",
    part: "E — Scaling Reads",
    ...
    l5Pattern: null,
    question:
      "An interviewer asks: 'For Instagram, roughly what fraction of database operations are writes?' Without exact data, what's a reasonable estimate, and what does the answer imply for scaling strategy?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The pattern's worked example is exactly this: 'Consider Instagram: when you open the app, you see dozens of photos requiring hundreds of database queries for metadata, user info, and engagement data. Meanwhile, you might only post once per day.' The point isn't the exact ratio — it's that read-write asymmetry is huge, so the scaling toolkit is asymmetric (replicas + caches + CDN, not sharding for writes).",
    interviewScript:
      "In your interview, say: 'Reads dominate by orders of magnitude here. A user opens the feed dozens of times a day; each open fires hundreds of queries. Writes — posts, likes — are maybe 1% of operations or less. That asymmetry tells me my scaling investments belong on the read side: indexing, replicas, and caching, in that order.'",
    proTip:
      "When asked for ratios, anchor on the user behavior — 'how many feed opens vs how many posts per day' — and let the math fall out. Interviewers want the reasoning, not the exact number.",
  },
  {
    id: "qE3",
    part: "E — Scaling Reads",
    ...
    l5Pattern: null,
    question:
      "Asked to scale reads on a slow query, a candidate's first move is: 'I'll put Redis in front of the database.' What's the more senior progression, and why does the order matter?",
    options: [
  ...
  },
  {
    ...
    l5Pattern: null,
    question:
      "A candidate proposes adding 5 read replicas to scale a slow feed-rendering query. The interviewer asks, 'Have you looked at the queries themselves?' Why is the question pointed?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The first step in the pattern's progression is in-DB optimization — indexing and denormalization. Replicas multiply the *current* query, including its inefficiencies. A 4-join feed query at 200ms remains 200ms across 5 replicas; a denormalized single-row read at 5ms beats it on one replica. Replicas are aggregate-throughput tools, not query-speed tools.",
    interviewScript:
      "In your interview, say: 'You're right — let me look at the query first. If it does 4 joins per feed item, the win is denormalizing into a feed-item table, not replicating. Replicas multiply throughput but not query speed. I'd index and denormalize first, then replicate.'",
    proTip:
      "Replicas multiply throughput; they don't improve per-query latency. If your latency is bad, fix the query first.",
  },
  {
    id: "qE4",
    part: "E — Scaling Reads",
    ...
    l5Pattern: null,
    question:
      "Candidate adds read replicas for scaling. The interviewer says: 'User A posts a comment. User A immediately refreshes the page. What might happen?'",
    options: [
  ...
  },
  {
    ...
    question:
      "A celebrity posts. Their profile is requested at 1M QPS for 30 seconds. The cache layer is a 50-node Redis cluster; the celebrity's profile is owned by one node. What breaks first, in what order, and why does adding more replicas not help in time?",
    options: [
  ...
  // ============================================================
  {
    ...
    l5Pattern: null,
    question:
      "A startup's monolithic Postgres handles 200 writes/sec comfortably. After a viral launch, they need to handle 200,000 writes/sec sustained. What broad class of strategy must they consider?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "The pattern's framing: 'individual database servers and storage systems hit hard limits' at high write QPS. The three strategies named are horizontal sharding, vertical partitioning, and burst handling. Recognizing this *as* a write-scaling problem — and naming the strategies — is the entry point.",
    interviewScript:
      "In your interview, say: 'At 200K writes/sec one server is no longer enough — vertical scaling will plateau. The toolkit is horizontal sharding, vertical partitioning, and burst handling via queues, load shedding, or batching. Which I reach for first depends on the workload shape — sustained throughput vs spikes.'",
    proTip:
      "Always name all three legs of the toolkit (sharding, partitioning, burst handling) before picking one. Interviewers probe the leg you didn't name.",
  },
  {
    id: "qF2",
    part: "F — Scaling Writes",
    ...
    l5Pattern: null,
    question:
      "A candidate says, 'we'll partition the database.' The interviewer asks: 'horizontal or vertical?' For an app where the orders table is enormous but the rest of the schema (users, products, settings) is fine, which fits, and what's the difference?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The pattern names both strategies as distinct. Horizontal sharding splits one large table by row across servers — same schema, more servers. Vertical partitioning splits the data model by *type* — orders here, analytics there. For a single hot table, horizontal is the lever; for a system where different parts of the schema have different scaling profiles, vertical is. The two are complementary.",
    interviewScript:
      "In your interview, say: 'Horizontal sharding splits the orders table by row across servers — same schema. Vertical partitioning would put different tables on different DBs. Here it's the orders table that's hot, so horizontal sharding by user_id (or order_id with co-location keys) is the move. I'd leave the rest of the schema on the original DB.'",
    proTip:
      "Use the words 'horizontal' and 'vertical' precisely. Conflating them is a fast tell that the candidate is at surface depth.",
  },
  {
    id: "qF3",
    part: "F — Scaling Writes",
    ...
    l5Pattern: null,
    question:
      "A candidate shards a chat service by `chat_id`. The interviewer asks: 'What happens to a query like \"show all chats for user X\"?'",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The pattern's explicit guidance: 'selecting good partition keys that distribute load evenly while keeping related data together.' The trade-off is real because related-by-write and related-by-read are often different relations. Sharding by chat_id co-locates all writes for one chat (great for a chat's hot path) but scatters a user's chats across shards (terrible for a user's chat list). The senior signal is articulating the trade-off, not finding a magical key that wins both.",
    interviewScript:
      "In your interview, say: 'Sharding by chat_id co-locates writes within a chat — the hot path — but means \"all chats for user X\" is a fan-out across all shards. The trade-off is unavoidable: chat_id wins for chat operations, user_id wins for user views. Two ways out: pick the key that matches the dominant access pattern and accept fan-out for the other, or maintain a secondary user→chat-list index in a separate store.'",
    proTip:
      "Partition keys encode workload assumptions. State the assumption explicitly when you pick one — 'I'm sharding by X because Y is the dominant access pattern' — and you've prevented the most common interview gotcha.",
  },
  {
    id: "qF4",
    part: "F — Scaling Writes",
    ...
    l5Pattern: null,
    question:
      "A payments system normally handles 1K writes/sec; a Black Friday spike hits 50K writes/sec. The candidate proposes: 'buffer the surge in a write queue.' The interviewer asks: 'What if the surge is sustained for 6 hours?'",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The pattern names both tools — write queues for spikes, load shedding for sustained overload. They're complementary, not interchangeable. A queue is a smoothing mechanism; if the long-run incoming rate exceeds drain rate, the queue grows without bound (or is dropped, which is load shedding by accident). Senior+ designs articulate when each tool fits.",
    interviewScript:
      "In your interview, say: 'Queues smooth temporary spikes — they assume drain capacity returns. For a 6-hour sustained surge, the queue grows unbounded. The right combination is queue for short bursts, plus load shedding for sustained overload — drop analytics writes, keep payment writes. Defining write priorities upfront is part of the design.'",
    proTip:
      "Always pair queues with load shedding when answering burst-handling questions. Either alone is incomplete.",
  },
  {
    id: "qF5",
    part: "F — Scaling Writes",
    ...
    question:
      "A candidate has designed a multi-region order system: sharded by user_id, batching writes for efficiency, optimized for the write path. The interviewer says: 'Now flip the workload — what if 90% of traffic becomes *reads* of order history rather than writes?' Why does the entire partitioning strategy potentially need to change?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "This is the trade-off-inversion canon: 'what if the requirement flips?' forces switching the chosen pattern. user_id sharding optimizes for the write-co-location and per-user-read access patterns. A read-heavy workload that scans across users (fraud, analytics, leaderboards) has fundamentally different access patterns and may want a CQRS read store, time-partitioning, or a different shard key entirely. The senior+ skill is recognizing that partition-key choices are workload-conditional and re-decomposing when the workload changes.",
    interviewScript:
      "In your interview, say: 'My current sharding by user_id was tuned for write co-location and per-user reads. If 90% of traffic is now cross-user analytics-style reads, fanning out across all shards every read is the wrong shape. I'd build a CQRS read store — denormalized, time-partitioned, fed by CDC from the user-sharded primary — and serve cross-user reads from there. The write side stays user-sharded; the read side gets its own optimal layout. Partition keys encode workload assumptions, and the workload just changed.'",
    proTip:
      "When workload assumptions flip, the right move is rarely tweaking the existing layout — it's adding a second layout (CQRS) tuned for the new workload. State this explicitly as the trade-off.",
  },
];
...
const PART_COUNTS_PLACEHOLDER = null; // computed below after array is fully populated
