  ...
  "questions": [
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "HTTP over TCP — universally supported, well-understood, and handles the vast majority of use cases"
        },
        {
          "label": "B",
          "text": "UDP for everything — it's faster than TCP and more modern"
        },
        {
          "label": "C",
          "text": "WebSockets — they're modern and support real-time applications"
        },
        {
          "label": "D",
          "text": "gRPC always — binary serialization is the obvious win"
        }
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "Server-Sent Events (SSE) — the data flow is server→client only, and SSE rides standard HTTP infrastructure with a one-time client handshake. It's simpler than WebSockets when the client doesn't need to push"
        },
        {
          "label": "C",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "POST /getUser with body { user_id: 123 }"
        },
        {
          "label": "B",
          "text": "GET /users/{id}"
        ...
        {
          "label": "C",
          "text": "WebSocket message of type 'fetch_user'"
        },
        {
          "label": "D",
          "text": "gRPC method UserService.Get(id)"
        }
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Relational (Postgres, MySQL) — you have clear entity relationships, structured data, transactions for the order flow, and foreign-key integrity. The article: relational works great when you have 'structured data with clear relationships and need strong consistency'"
        },
        {
          "label": "B",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "An index was added on the `email` column. Without an index, the database scans every one of the 10M rows comparing the email column. With an index (typically a B-tree), the database walks a balanced tree in O(log n) and jumps directly to the matching row"
        },
        {
          "label": "B",
    ...
    },
    {
      ...
      "options": [
        {
          "label": "A",
          "text": "Service queries Redis for `user:42` → MISS → service queries Postgres → gets the user → writes the user back to Redis with a TTL → returns the user. The next request for user 42 hits Redis directly and skips Postgres entirely. The article: 'On a read, check the cache first. If the data is there, return it. If not, query the database, store the result in the cache with a TTL, and return it'"
        },
        {
          "label": "B",
    ...
    },
    {
      ...
      "options": [
        ...
        {
          "label": "B",
          "text": "About 90% — almost every key now maps to a different node because the modulus changed from 10 to 11. For a cache, 90% of keys would suddenly miss; for a sharded DB, 90% of data would need to physically move. The article: 'With simple modulo hashing, adding one server to a 10-server cluster means moving roughly 90% of your data.' This is why naive modulo hashing isn't used for cluster scaling"
        },
        {
          "label": "C",
