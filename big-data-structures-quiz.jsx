...
const QUESTIONS = [
  ...
  },
  {
    ...
    question:
      "A Bloom filter uses k=3 hash functions and a bit array of size m=12. Elements X, Y, and Z are inserted. When checking membership for element W, all k bit positions for W are found to be set to 1. Which statement is MOST accurate?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "Your system uses a Bloom filter to check if a user has already received a promotional email. If the filter says 'probably yes,' you skip sending. If it says 'definitely no,' you send the email. After deployment, which failure mode will you observe?",
    options: [
      {
        label: "A",
    ...
    interviewScript:
      "In your interview, say: 'Bloom filters only produce false positives, never false negatives. In this design, that means we'll occasionally skip users who should receive the email — the filter falsely claims they already got it. We need to verify this failure mode is acceptable for the business, or add a secondary check for the positive case.'",
    proTip:
      "This is a classic interview trap. Always map the Bloom filter's error type to the business impact. False positives might be acceptable for web crawling (recrawl a page) but devastating for billing (skip a charge).",
  },
  {
    id: "q4",
    subtopic: "Bloom Filter Mechanics",
    ...
    question:
      "A Bloom filter has been running in production for 6 months, accumulating insertions without any maintenance. The ops team notices the false positive rate has climbed from the designed 1% to over 40%. They propose three fixes. Which approach is MOST appropriate?",
    options: [
      {
        label: "A",
  ...
  // === BLOOM FILTER USE-CASES & PITFALLS ===
  {
    ...
    style: "Scenario-based trade-off",
    question:
      "You're building a web crawler that needs to avoid recrawling URLs. Your Bloom filter has a 2% false positive rate. A colleague argues this means 2% of the internet will never be crawled. What's wrong with this reasoning, and what is the ACTUAL impact?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "Your team uses a Bloom filter to avoid unnecessary cache lookups. The cache uses TTL-based eviction. After running for a week, you notice the Bloom filter's benefit has decreased significantly — cache miss latency is barely improving. What's the root cause, and what's the best mitigation?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Gotcha/trap",
    question:
      "A candidate proposes using a Bloom filter for a user blocklist service: 'We'll add blocked user IDs to the filter. When a message is sent, we check the filter — if the sender is blocked, we drop the message.' The interviewer nods and asks: 'What happens when a user unblocks someone?' What is the critical flaw?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "A candidate designing a payment processing system proposes: 'We'll use a Bloom filter to deduplicate transaction IDs. When a transaction comes in, we check the filter — if it's probably seen, we reject it as duplicate. If definitely not seen, we process it.' What's the most dangerous flaw?",
    options: [
      {
        label: "A",
  ...
  // === COUNT-MIN SKETCH MECHANICS ===
  {
    ...
    style: "Implementation-specific nuance",
    question:
      "In a Count-Min Sketch with depth d=3 and width w=8, you increment element X five times. When querying X's count, the three counters (one per row) show values [7, 5, 9]. What can you conclude about X's true count?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 1,
    explanation:
      "Count-Min Sketch provides an UPPER BOUND estimate by taking the MINIMUM across all rows. Each counter can only be equal to or greater than the true count (because collisions only add, never subtract). The minimum of [7, 5, 9] = 5, which is the tightest upper bound we can provide. Option A is wrong — averaging doesn't give meaningful bounds. Option C is partially right about the value but wrong about the reasoning — we can't prove zero collisions; we only know the minimum is our best estimate. Option D incorrectly treats this as a range estimate.",
    interviewScript:
      "In your interview, say: 'Count-Min Sketch only overestimates, never underestimates, because hash collisions can only inflate counters. By taking the minimum across all d rows, we get the tightest upper bound on the true count. The name literally encodes the algorithm — COUNT using the MINimum.'",
    proTip: null,
  },
  {
    id: "q9",
    ...
    style: "Failure analysis",
    question:
      "Your Count-Min Sketch has been running for a month tracking video view counts. You notice that unpopular videos (true count ~10) are being reported with counts of 50,000+. Meanwhile, the most popular video (true count ~1B) shows a count of 1,000,002,347. Why is the error distribution so skewed?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "You're sizing a Count-Min Sketch for a real-time trending topics system. You have a fixed memory budget. Should you prioritize width (w) or depth (d) of the sketch?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "You propose using Count-Min Sketch for LFU cache eviction tracking. Your interviewer pushes back: 'Redis implemented LFU but chose NOT to use Count-Min Sketch. Why might that be?' What's the strongest response?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Anti-pattern identification",
    question:
      "A team building a billing system proposes: 'We'll use Count-Min Sketch to track API call counts per customer for usage-based billing. It saves memory compared to per-customer counters.' What's the most critical problem with this proposal?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 0,
    explanation:
      "CMS only overestimates, meaning customers would sometimes be overcharged — a serious legal and trust issue. The content warns that CMS is inappropriate 'when other parts of the system depend on the counts' and that you need to 'bend your requirements to make it work.' Billing has zero tolerance for inaccuracy. Option B is also true (CMS requires known items for queries) but is secondary to the financial impact. Option C is solvable by creating a new sketch per cycle. Option D may be true in some cases but isn't the fundamental issue.",
    interviewScript:
      "In your interview, say: 'This is a classic case of optimizing the wrong thing. CMS saves space by sacrificing accuracy, but billing requires exact counts — there's no acceptable error tolerance when charging customers. The space savings are irrelevant when correctness is a legal requirement. Use a simple per-customer counter in a database.'",
    proTip: null,
  },
  {
    id: "q14",
    ...
    style: "Gotcha/trap",
    question:
      "You've deployed a Count-Min Sketch to track trending search terms. A product manager asks: 'Can you pull a report of ALL search terms and their approximate counts from the sketch?' What's the correct response?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 2,
    explanation:
      "This is a fundamental CMS limitation the content explicitly calls out: 'our sketch does not tell us which items are present after we hash them.' CMS is a lossy compression of counts — you can query 'what's the approximate count for term X?' but you cannot ask 'what terms have been counted?' The original items are destroyed by hashing. You need to maintain a separate list of known items. Option A is impossible — hash functions are one-way. Option B misunderstands how CMS works. Option D is partially right about the limitation but wrong about extracting top items from counters alone.",
    interviewScript:
      "In your interview, say: 'Count-Min Sketch requires known items for queries — it's a count oracle, not an inventory. The items themselves are destroyed by hashing. For a trending terms system, you'd pair CMS with a candidate list — perhaps a heap of terms whose CMS counts exceed a threshold — to maintain the set of known high-count items.'",
    proTip: null,
  },
  ...
  // === HYPERLOGLOG MECHANICS ===
  {
    ...
    style: "Implementation-specific nuance",
    question:
      "HyperLogLog estimates cardinality by observing the maximum number of leading zeros in hashed values. If the longest run of leading zeros observed is 12, approximately how many unique elements have been seen?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Failure analysis",
    question:
      "A naive implementation of the leading-zeros cardinality estimation (without buckets) gives you an estimate of 2^15 = 32,768 unique users. But you know the true count is ~500. What most likely happened, and how does HLL prevent this?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    question:
      "You need to count unique daily visitors to a website with approximately 100 million unique visitors per day. Your memory budget is 12KB. Can HyperLogLog solve this, and what error rate should you expect?",
    options: [
      {
        label: "A",
  ...
  // === HYPERLOGLOG USE-CASES ===
  {
    ...
    style: "Scenario-based trade-off",
    question:
      "Your analytics platform needs to support queries like 'How many unique users visited page X in the last 30 days?' with sub-second latency. You have 50 million pages and 500 million daily unique visitors. A hash set per page is infeasible. An engineer proposes HLL per page. What's the key operational challenge?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 0,
    explanation:
      "HLL's key operational advantage is mergeability — you can union two HLL sketches by taking the element-wise maximum of their registers. This enables a powerful pattern: maintain daily HLL sketches per page, and merge the last 30 to answer windowed queries. The challenge is managing this state efficiently at scale. Option B is wrong — HLL supports time-windowed queries precisely through this merge pattern. Option C is usually wrong for analytics — 1-2% error is widely accepted. Option D is wrong — all HLL instances can use the same hash function.",
    interviewScript:
      "In your interview, say: 'The key insight is HLL mergeability. We maintain daily HLL sketches per page, and to answer 30-day unique queries, we merge 30 daily sketches using register-wise maximum. The operational challenge is managing 50M × 30 days of HLL state efficiently, but at 12KB each, that's tractable with proper storage and caching.'",
    proTip: null,
  },
  {
    id: "q19",
    ...
    style: "Multi-hop reasoning",
    question:
      "You're building an anti-scraping system. You want to detect IP addresses that are scraping by visiting an unusually high number of DISTINCT pages. A colleague suggests using request count per IP. Why might HyperLogLog provide a BETTER signal, and what's the trade-off?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Decision framework application",
    question:
      "You're designing a caching layer and need to determine the working set size — how many unique keys are accessed over time — to properly size your cache. You consider three approaches: (1) log every key to a database and COUNT DISTINCT, (2) maintain a hash set of seen keys in memory, (3) use HyperLogLog. Under what conditions is HLL the right choice?",
    options: [
      {
        label: "A",
  ...
  // === APPROXIMATE QUANTILES ===
  {
    ...
    style: "Gotcha/trap",
    question:
      "You're monitoring API latency using fixed-width histogram buckets: [0-100ms, 100-200ms, 200-300ms, ..., 900-1000ms, 1000ms+]. 95% of requests are under 50ms. What's the fundamental problem with this bucket configuration?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Decision framework application",
    question:
      "You need to track latency percentiles for a service with highly variable response times: p50 is ~10ms but p99 is ~30 seconds (extreme long tail). You're choosing between fixed-width, exponential, and dynamic histogram buckets. Which approach is most appropriate?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Implementation-specific nuance",
    question:
      "Your histogram implementation uses cumulative counters (each bucket stores the count of ALL values up to its boundary) instead of per-bucket counters. What are the implications for read and write performance?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Scenario-based trade-off",
    question:
      "Your SLO states: '99% of API requests must complete within 200ms.' You're using Prometheus histograms to monitor compliance. Your histogram has buckets at [10, 25, 50, 100, 250, 500, 1000]ms. The 99th percentile is estimated at 180ms, just under the 200ms SLO. Should you be confident the SLO is being met?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Scenario-based trade-off",
    question:
      "Your auto-scaling system triggers scale-up when p95 CPU utilization exceeds 80%. You're using exponential histogram buckets for CPU: [0, 10, 20, 40, 60, 80, 90, 95, 100]%. The system reports p95 CPU at 78% and does NOT scale up. During a sudden traffic spike, the service goes down. What's the most likely root cause?",
    options: [
      {
        label: "A",
  ...
  // === SPACE-ACCURACY TRADEOFFS ===
  {
    ...
    question:
      "You're designing a real-time analytics pipeline that needs to answer three questions simultaneously for each endpoint: (1) How many unique users hit this endpoint? (2) What are the top-10 most frequent error codes? (3) What's the p99 latency? Your per-endpoint memory budget is 20KB. How would you allocate it?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 1,
    explanation:
      "Each probabilistic data structure is designed for a specific problem: HLL for cardinality, CMS for frequency, histograms for quantiles. The key insight is how remarkably memory-efficient each is. HLL needs only 1.5KB for ~2% error. CMS needs minimal space for a small set of error codes. Histograms need only a counter per bucket. Together they fit easily in 20KB with room to spare. Option A is wrong — CMS can't do cardinality or percentiles. Option C over-allocates to HLL (diminishing returns). Option D wildly overestimates memory needs.",
    interviewScript:
      "In your interview, say: 'Each question maps to a specialized data structure: HLL for unique users at 1.5KB, CMS for error code frequency at 2-4KB, and an exponential histogram for p99 at under 200 bytes. These structures compose naturally and fit well within 20KB. The remaining budget goes to improving CMS accuracy, since error code frequency is the most collision-prone.'",
    proTip:
      "In interviews, demonstrating that you can compose multiple probabilistic data structures within a tight memory budget shows deep understanding of their space characteristics.",
  },
  {
    id: "q27",
    subtopic: "Space-Accuracy Tradeoffs",
    ...
    style: "Estimation-backed reasoning",
    question:
      "You need to store 1 billion 4-byte element IDs with a 1% false positive rate. A Bloom filter for this requires approximately 1GB. A well-optimized hash set requires approximately 5GB. Your system has 3GB of available RAM. What's the best approach?",
    options: [
      {
        label: "A",
  ...
  // === WHEN NOT TO USE ===
  {
    ...
    style: "Anti-pattern identification",
    question:
      "A candidate designing a social media feed says: 'I'll use a Bloom filter per user to track which posts they've already seen, so we don't show duplicates in their feed.' The system has 100 million users with an average of 500 seen posts each. What's wrong with this proposal?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    style: "Anti-pattern identification",
    question:
      "During a system design interview, a candidate reaches for HyperLogLog every time uniqueness counting comes up — including counting unique items in a dataset of ~10,000 elements that easily fits in memory. What signal does this send to the interviewer?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 1,
    explanation:
      "The content opens with this warning: 'Employing a bloom filter when a simple hash table would suffice is a red flag for an interviewer.' The same principle applies to all probabilistic structures. For 10K items, a hash set is trivial — ~80KB, O(1) operations, exact results. Using HLL adds ~2% error for no space benefit. The interviewer is testing practical engineering judgment, not just knowledge of exotic data structures. Option D is wrong — HLL works at any scale, it's just unnecessary at small scales.",
    interviewScript:
      "In your interview, say: 'I'd use a simple hash set here. With 10,000 items, we're talking about kilobytes of memory — there's no space constraint that justifies a probabilistic approach. HyperLogLog is powerful when you're counting billions of uniques with tight memory budgets, but using it for 10K items adds approximation error with zero benefit. Choosing the right tool for the scale is key.'",
    proTip:
      "In interviews, demonstrating when NOT to use an advanced technique is often more impressive than knowing the technique itself. It shows practical engineering judgment.",
  },
  ...
  // === THIN SUBTOPICS ===
  {
    ...
    style: "Gotcha/trap",
    question:
      "You're sizing a Bloom filter for 100 million elements with a 0.1% false positive rate. Your colleague claims the filter will be 'tiny — just a few megabytes.' Based on the relationship between element count, FPR, and filter size, is this claim reasonable?",
    options: [
      {
        label: "A",
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      "HLL at 1.5KB per instance with 1M pages = 1.5GB total. Hash sets: 1M pages × 100K unique IDs × 8 bytes per ID = 800GB. That's a 99.8% reduction. The content states 1.5KB gives ~2% error, which is sufficient for analytics. Option A miscalculates hash set size (800GB, not 800TB). Option C uses 12KB per HLL, which is valid but unnecessary — 1.5KB already gives useful accuracy. Option D fabricates degradation with more instances.",
    interviewScript:
      "In your interview, say: 'At 1.5KB per HLL with 2% error, tracking 1M pages costs only 1.5GB total versus 800GB for hash sets. That's a 500x reduction. The ~2% error is perfectly acceptable for page analytics. This is exactly the scale where HLL transforms what's architecturally feasible.'",
    proTip: null,
  },
  {
    id: "q32",
    ...
    style: "Interviewer pushback",
    question:
      "A candidate proposes dynamic histograms that adapt bucket boundaries to the observed data distribution for a latency monitoring system. The interviewer asks: 'What's the operational cost of dynamic boundaries?' What's the best response?",
    options: [
      {
        label: "A",
  ...
  // === CROSS-SUBTOPIC BRIDGES ===
  {
    ...
    style: "Cross-subtopic bridge",
    question:
      "You're designing a system to detect and rate-limit abusive API clients. You need to both (a) check if a client IP has been seen before (new vs returning) and (b) estimate how many requests they've made. A colleague suggests: 'Just use Count-Min Sketch — it handles both membership and counting.' Why is this subtly wrong?",
    options: [
      {
        label: "A",
    ...
    correctIndex: 1,
    explanation:
      "CMS CAN technically check membership (count > 0 means possibly present, count = 0 means definitely absent), but its primary purpose is counting, and its design is optimized for count accuracy, not membership testing. A Bloom filter is specifically optimized for membership queries with lower memory for the same false positive rate. More importantly, the content emphasizes CMS requires 'known items' — you query CMS with a specific item. Both structures complement each other. Option A oversimplifies. Option C is wrong — CMS doesn't have false negatives for the membership check. Option D is wrong — CMS works with any hashable input.",
    interviewScript:
      "In your interview, say: 'While CMS can technically answer membership queries, it's optimized for counting. For a clean design, I'd use a Bloom filter for the membership check — it's more space-efficient for that specific question — and CMS for the count estimation. Each structure excels at its specific task.'",
    proTip: null,
  },
  {
    id: "q35",
  ...
  },
  {
    ...
    style: "Decision framework application",
    question:
      "Your system receives a stream of user IDs and needs to answer two questions: (1) 'Has user X been seen in this stream?' (membership) and (2) 'How many unique users have we seen total?' (cardinality). You can only afford ONE data structure. Which do you choose?",
    options: [
      {
        label: "A",
    ...
    interviewScript:
      "In your interview, say: 'HLL is a summary statistic — it tracks cardinality but can't answer per-element membership queries. Bloom filters can do membership directly and estimate cardinality from bit saturation. If I can only pick one and membership is important, I'd choose the Bloom filter and derive cardinality as a secondary metric. If cardinality precision matters more, I'd need HLL and accept that membership queries require a separate mechanism.'",
    proTip: null,
  },
  {
    id: "q37",
    ...
    style: "Multi-hop reasoning",
    question:
      "You're building a dashboard showing 'Top 10 endpoints by p99 latency.' You have per-endpoint histograms for latency AND a Count-Min Sketch tracking request counts per endpoint. An engineer asks: 'Why not just use CMS to track p99 latency directly?' What's the fundamental misunderstanding?",
    options: [
      {
        label: "A",
...
const SUBTOPIC_ORDER = [
  ...
  "Approximate Quantiles",
  "Approximate Quantiles Mechanics",
  "Approximate Quantiles Use-Cases",
  "Dynamic Histograms",
  "Space-Accuracy Tradeoffs",
  "When NOT to Use Probabilistic Structures",
  "Bloom Filter × Count-Min Sketch",
  "HyperLogLog × Approximate Quantiles",
  "Bloom Filter × HyperLogLog",
  "Count-Min Sketch × Approximate Quantiles",
];

const TIER_COLORS = {
  Heavy: { bg: "bg-red-900/30", text: "text-red-400", border: "border-red-700" },
  Medium: { bg: "bg-amber-900/30", text: "text-amber-400", border: "border-amber-700" },
  Thin: { bg: "bg-blue-900/30", text: "text-blue-400", border: "border-blue-700" },
  Bridge: { bg: "bg-purple-900/30", text: "text-purple-400", border: "border-purple-700" },
};
...
function getSubtopicStats() {
  const stats = {};
  QUESTIONS.forEach((q) => {
    if (!stats[q.subtopic]) stats[q.subtopic] = { count: 0, tier: q.tier };
    stats[q.subtopic].count++;
  });
  return stats;
}
...
function getGrade(pct) {
  ...
  if (pct >= 75) return { label: "Strong Senior", desc: "solid, minor gaps to close", color: "text-blue-400" };
  if (pct >= 60) return { label: "SDE2-level", desc: "review the weak areas below", color: "text-amber-400" };
  return { label: "Needs deep review", desc: "revisit the fundamentals", color: "text-red-400" };
}

export default function BigDataStructuresQuiz({ quizSlug = 'advanced-data-structures-big-data' }) {
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if (e.key === "Enter" && selectedOption !== null && confidence !== null) submitAnswer();
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  }, [screen, currentQ, submitted, selectedOption, confidence]);
  ...
  const results = useMemo(() => {
    ...
    const pct = totalAnswered > 0 ? Math.round((correctCount / allQs.length) * 100) : 0;
    ...
    allQs.forEach((q) => {
      if (!subtopicResults[q.subtopic])
        subtopicResults[q.subtopic] = { total: 0, correct: 0, tier: q.tier, questions: [] };
      subtopicResults[q.subtopic].total++;
      const ans = answers[q.id];
      if (ans?.correct) subtopicResults[q.subtopic].correct++;
      subtopicResults[q.subtopic].questions.push({ ...q, answer: ans });
    });

    // Confidence analysis
    const luckyGuesses = allQs.filter(
      (q) => answers[q.id]?.correct && answers[q.id]?.confidence === "Guessing"
    );
    const overconfidentMisses = allQs.filter(
      (q) => answers[q.id] && !answers[q.id].correct && answers[q.id].confidence === "Very Confident"
    );

    // Incorrect & flagged
    const incorrect = allQs.filter((q) => answers[q.id] && !answers[q.id].correct);
    const flaggedQs = allQs.filter((q) => flagged.has(q.id));
  ...
  }, [screen, answers, flagged]);

  // LANDING SCREEN
  if (screen === "landing") {
    const stats = getSubtopicStats();
    const tiers = { Heavy: [], Medium: [], Thin: [], Bridge: [] };
    Object.entries(stats).forEach(([name, { count, tier }]) => {
      if (tiers[tier]) tiers[tier].push({ name, count });
    });
      ...
      <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-900/40 text-red-400 text-sm font-medium mb-4">
              <Zap className="w-4 h-4" /> FAANG SDE2 — Hard
            </div>
            <h1 className="text-3xl font-bold mb-2">Data Structures for Big Data</h1>
            <p className="text-gray-400">
              Bloom Filters, Count-Min Sketch, HyperLogLog & Approximate Quantiles — scenario-based
              questions calibrated for system design interviews.
            </p>
            <div className="flex items-center justify-center gap-6 mt-4 text-sm text-gray-500">
              ...
              <span className="flex items-center gap-1">
                <Timer className="w-4 h-4" /> ~{Math.round((QUESTIONS.length * 75) / 60)} min
              </span>
            </div>
          </div>
              ...
              ([tier, items]) =>
                items.length > 0 && (
                  <div key={tier} className="mb-3">
                    <div
                      className={`text-xs font-semibold uppercase tracking-wider mb-1.5 ${TIER_COLORS[tier].text}`}
                    >
                      {tier} ({items.reduce((s, i) => s + i.count, 0)} questions)
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {items.map((item) => (
                        <span
                          key={item.name}
                          className={`text-xs px-2 py-1 rounded ${TIER_COLORS[tier].bg} ${TIER_COLORS[tier].text} border ${TIER_COLORS[tier].border}`}
                        >
          ...
          <div className="flex gap-3 mb-4">
            ...
            >
              <List className="w-4 h-4" /> Section Order
            </button>
          </div>

          <button
            onClick={() => startQuiz(QUESTIONS)}
            className="w-full py-4 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-lg transition-all"
          >
            Start Quiz
          </button>
        </div>
  ...
  if (screen === "quiz" && currentQ) {
    ...
    const timerBg = timer <= 15 ? "bg-red-900/30" : timer <= 30 ? "bg-amber-900/30" : "bg-gray-800";
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${timerBg}`}>
              ...
              <span className={`font-mono font-semibold ${timerColor}`}>{formatTime(timer)}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            <span
              className={`text-xs px-2 py-0.5 rounded ${TIER_COLORS[currentQ.tier]?.bg || "bg-gray-800"} ${
                TIER_COLORS[currentQ.tier]?.text || "text-gray-400"
              } border ${TIER_COLORS[currentQ.tier]?.border || "border-gray-700"}`}
            >
              {currentQ.subtopic}
            </span>
            ...
            {currentQ.options.map((opt, i) => {
              ...
              if (submitted) {
                if (i === currentQ.correctIndex) {
                  optClasses += " bg-emerald-900/30 border-emerald-600 text-emerald-200";
                } else if (
                  i === selectedOption &&
                  selectedOption !== currentQ.correctIndex
                ) {
                  optClasses += " bg-red-900/30 border-red-600 text-red-200";
                } else {
                  optClasses += " bg-gray-900/50 border-gray-800 text-gray-500 cursor-default";
                }
              } else if (i === selectedOption) {
                optClasses += " bg-indigo-900/30 border-indigo-500 text-indigo-200";
              } else {
                optClasses +=
            ...
            {!submitted ? (
              <>
                <button
                  ...
                  className="flex-1 py-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-800 disabled:text-gray-600 text-white font-semibold transition-all"
                >
                  Submit Answer
                </button>
                <button
                  onClick={skipQuestion}
                  className="px-4 py-3 rounded-lg bg-gray-800 border border-gray-700 text-gray-400 hover:text-gray-200 hover:border-gray-500 transition-all"
                  title="Skip"
                >
              ...
              <>
                <button
                  ...
                  className="flex-1 py-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg bg-indigo-900/20 border border-indigo-800">
                <div className="flex items-center gap-2 text-indigo-300 font-semibold mb-2">
                  <Target className="w-5 h-5" /> Interview Script
                </div>
                <p className="text-sm text-indigo-200/80 leading-relaxed italic">
                  {currentQ.interviewScript}
                </p>
              </div>
              ...
              {currentQ.proTip && (
                <div className="p-4 rounded-lg bg-amber-900/20 border border-amber-800">
                  <div className="flex items-center gap-2 text-amber-300 font-semibold mb-2">
                    <Award className="w-5 h-5" /> Pro Tip
                  </div>
                  <p className="text-sm text-amber-200/80 leading-relaxed">{currentQ.proTip}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-5 mb-6">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <BarChart3 className="w-4 h-4" /> Subtopic Breakdown
            </h3>
            {Object.entries(subtopicResults).map(([name, data]) => {
              const subPct = data.total > 0 ? Math.round((data.correct / data.total) * 100) : 0;
              const barColor = subPct >= 70 ? "bg-emerald-500" : subPct >= 50 ? "bg-amber-500" : "bg-red-500";
              return (
                    ...
                    <span className="text-gray-300 flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs px-1.5 py-0.5 rounded ${
                          TIER_COLORS[data.tier]?.bg || "bg-gray-800"
                        } ${TIER_COLORS[data.tier]?.text || "text-gray-400"}`}
                      >
                        {data.tier}
                      </span>
                    </span>
                    <span className="text-gray-400">
                      {data.correct}/{data.total} ({subPct}%)
                    </span>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-4">
              <h3 className="text-sm font-semibold text-amber-400 mb-3 flex items-center gap-2">
                <AlertTriangle className="w-4 h-4" /> Lucky Guesses ({luckyGuesses.length})
              </h3>
              <p className="text-xs text-gray-500 mb-2">
                Correct answers with "Guessing" confidence — hidden weak spots
              </p>
              {luckyGuesses.length === 0 ? (
                <p className="text-sm text-gray-500 italic">None — you knew what you knew!</p>
              ) : (
                <ul className="space-y-1">
                  {luckyGuesses.map((q) => (
                    <li key={q.id} className="text-sm text-gray-400 truncate">
                      • {q.subtopic}: {q.question.slice(0, 60)}...
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-4">
              <h3 className="text-sm font-semibold text-red-400 mb-3 flex items-center gap-2">
                <XCircle className="w-4 h-4" /> Overconfident Misses ({overconfidentMisses.length})
              </h3>
              <p className="text-xs text-gray-500 mb-2">
                Wrong answers with "Very Confident" — dangerous misconceptions
              </p>
              {overconfidentMisses.length === 0 ? (
                <p className="text-sm text-gray-500 italic">None — well calibrated!</p>
              ) : (
                <ul className="space-y-1">
          ...
          {incorrect.length > 0 && (
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-5 mb-6">
              <h3 className="text-sm font-semibold text-red-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                <XCircle className="w-4 h-4" /> Incorrect Questions ({incorrect.length})
              </h3>
                    ...
                    <div key={q.id} className="border-b border-gray-800 pb-4 last:border-0">
                      ...
                      {ans && !ans.timedOut && (
                        <p className="text-xs text-red-400 mb-1">
                          Your answer: {q.options[ans.selected]?.label} — Confidence: {ans.confidence}
                        </p>
                      )}
                      {ans?.timedOut && (
                        <p className="text-xs text-red-400 mb-1">Timed out</p>
                      )}
                      <p className="text-xs text-emerald-400 mb-2">
                        Correct: {q.options[q.correctIndex].label}
                      </p>
          ...
          {flaggedQs.length > 0 && (
            <div className="bg-gray-900 rounded-lg border border-gray-800 p-5 mb-6">
              <h3 className="text-sm font-semibold text-amber-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                <Flag className="w-4 h-4" /> Flagged for Review ({flaggedQs.length})
              </h3>
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {flaggedQs.map((q) => (
                  <div key={q.id} className="border-b border-gray-800 pb-4 last:border-0">
                    ...
                    <p className="text-sm text-gray-300 mb-2">{q.question}</p>
                    <p className="text-xs text-emerald-400 mb-2">
                      Correct: {q.options[q.correctIndex].label}. {q.options[q.correctIndex].text}
                    </p>
                    <p className="text-xs text-gray-500">{q.explanation}</p>
                    {q.proTip && (
                      <p className="text-xs text-amber-400 mt-1">Pro Tip: {q.proTip}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {incorrect.length > 0 && (
              <button
                ...
                className="flex-1 py-3 rounded-lg bg-red-900/40 border border-red-700 text-red-300 hover:bg-red-900/60 font-semibold transition-all flex items-center justify-center gap-2"
              >
            ...
            {weakSubtopics.length > 0 && (
              <button
                ...
                className="flex-1 py-3 rounded-lg bg-amber-900/40 border border-amber-700 text-amber-300 hover:bg-amber-900/60 font-semibold transition-all flex items-center justify-center gap-2"
              >
                <BarChart3 className="w-4 h-4" /> Retry Weak Subtopics
              </button>
            )}
            <button
              onClick={() => {
              ...
              className="flex-1 py-3 rounded-lg bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700 font-semibold transition-all flex items-center justify-center gap-2"
            >
