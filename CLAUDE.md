...
3. **Exhaustive coverage.** Every concept in the source must be covered by at least one question. The Coverage Manifest at the top of the output `.jsx` file is the enforcement mechanism.
...
6. Emit the `.jsx` file matching the existing pattern (e.g. `caching-quiz.jsx`).
7. Register the file in **`App.jsx`** (`componentsMap`) and **`quiz-state.json`**.
...
The first new quiz you generate may need to extend `QuizEngine.jsx` with: `difficulty` chip rendering, a "Ladder" play mode, per-tier timers (60/90/90/120/150s), and an "Accuracy by Tier" results breakdown. All extensions must be **backward-compatible** with the 19 existing quiz files that lack the `difficulty` field. Methodology Section 7 has details.

---

## Project layout (already in place)

- `QuizEngine.jsx` — shared quiz UI runtime (per-topic copies exist as `*-quiz.jsx`)
- `App.jsx` — router; `componentsMap` registers each quiz
- `quiz-state.json` — topic catalog with category, slug, status, output filename
- `quizProgressStore.js`, `useQuizProgress.js` — localStorage-backed progress
- `*-quiz.jsx` (one per topic) — quiz files following the established schema
...
- Don't break backward compatibility with the existing 19 quiz files when extending `QuizEngine.jsx`.
