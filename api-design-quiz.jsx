...
const SUBTOPIC_INFO = [
  { name: "REST Resource Modeling", tier: "heavy", count: 4 },
  { name: "REST HTTP Methods & Idempotency", tier: "heavy", count: 4 },
  { name: "REST Data Passing & Responses", tier: "heavy", count: 3 },
  { name: "GraphQL", tier: "heavy", count: 4 },
  { name: "Authentication & Authorization", tier: "heavy", count: 4 },
  { name: "RPC / gRPC", tier: "medium", count: 3 },
  { name: "Pagination", tier: "medium", count: 2 },
  { name: "API Type Selection", tier: "medium", count: 2 },
  { name: "Rate Limiting", tier: "medium", count: 2 },
  { name: "Versioning Strategies", tier: "thin", count: 1 },
  { name: "Interview Strategy / Time Management", tier: "thin", count: 1 },
  { name: "Real-time + API Selection", tier: "thin", count: 1 },
  { name: "Cross-subtopic Bridges", tier: "bridge", count: 4 }
];

const TIER_COLORS = {
  heavy: { bg: "bg-red-900/30", text: "text-red-400", border: "border-red-800" },
  medium: { bg: "bg-amber-900/30", text: "text-amber-400", border: "border-amber-800" },
  thin: { bg: "bg-blue-900/30", text: "text-blue-400", border: "border-blue-800" },
  bridge: { bg: "bg-purple-900/30", text: "text-purple-400", border: "border-purple-800" }
};
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-900/40 border border-red-700 text-red-400 text-sm font-medium mb-4">
            <Zap size={14} />
            FAANG SDE2 — Hard
          </div>
          ...
          <p className="text-gray-400 text-lg">System Design Interview Quiz</p>
          <p className="text-gray-500 mt-2 text-sm">
            {QUESTIONS.length} questions · ~{totalTime} min · Covers REST, GraphQL, gRPC, Auth, Pagination & more
          </p>
        </div>

        <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Coverage Breakdown</h2>
          {["heavy", "medium", "thin", "bridge"].map(tier => {
            const items = SUBTOPIC_INFO.filter(s => s.tier === tier);
            if (!items.length) return null;
            const label = tier === "bridge" ? "Cross-Subtopic" : tier.charAt(0).toUpperCase() + tier.slice(1);
            const c = TIER_COLORS[tier];
            return (
              <div key={tier} className="mb-4">
                <div className={`text-xs font-semibold uppercase tracking-wider mb-2 ${c.text}`}>{label}</div>
                <div className="flex flex-wrap gap-2">
                  {items.map(s => (
                    <span key={s.name} className={`px-2 py-1 rounded text-xs ${c.bg} ${c.text} border ${c.border}`}>
                      {s.name} ({s.count})
                    </span>
                  ))}
                </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Quiz Mode</h2>
          ...
          <p className="text-xs text-gray-500 mt-2 text-center">
            {mode === "shuffled" ? "Randomized order — simulates interview unpredictability" : "Grouped by subtopic in content order"}
          </p>
        </div>
...
function QuestionScreen({ question, index, total, onAnswer, onSkip, onFlag, isFlagged, timeLeft, selectedOption, confidence, setSelectedOption, setConfidence, submitted }) {
  const timerColor = timeLeft <= 15 ? "text-red-400" : timeLeft <= 30 ? "text-amber-400" : "text-green-400";
  const timerBg = timeLeft <= 15 ? "bg-red-900/30" : timeLeft <= 30 ? "bg-amber-900/30" : "bg-green-900/30";
  const progress = ((index + 1) / total) * 100;

  const subtopicTier = SUBTOPIC_INFO.find(s => s.name === question.subtopic)?.tier || "medium";
  const tc = TIER_COLORS[subtopicTier] || TIER_COLORS.medium;
  ...
  useEffect(() => {
    function handleKey(e) {
      ...
      if (e.key === "Enter" && selectedOption !== null && confidence !== null) onAnswer();
    }
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-gray-500">Question {index + 1} of {total}</span>
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full ${timerBg} ${timerColor} font-mono text-sm font-semibold`}>
            <Clock size={14} />
            {timeLeft}s
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className={`px-2 py-0.5 rounded text-xs font-medium ${tc.bg} ${tc.text} border ${tc.border}`}>
            {question.subtopic}
          </span>
        ...
        <div className="space-y-3 mb-4">
          {question.options.map((opt, i) => {
            let border = "border-gray-800 hover:border-gray-600";
            let bg = "bg-gray-900";
            if (submitted) {
              if (i === question.correct) { border = "border-green-600"; bg = "bg-green-900/20"; }
              else if (i === selectedOption && i !== question.correct) { border = "border-red-600"; bg = "bg-red-900/20"; }
              else { border = "border-gray-800 opacity-50"; }
            } else if (i === selectedOption) {
              border = "border-blue-500"; bg = "bg-blue-900/20";
            }
            return (
              <button
                key={i}
                ...
                disabled={submitted}
                className={`w-full text-left p-4 rounded-lg border ${border} ${bg} transition-all flex gap-3`}
              >
                <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-sm font-semibold ${
                  submitted && i === question.correct ? "bg-green-600 text-white" :
                  submitted && i === selectedOption ? "bg-red-600 text-white" :
                  i === selectedOption ? "bg-blue-600 text-white" :
        ...
        {!submitted && (
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              <button
                onClick={onSkip}
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-400 hover:text-gray-300 hover:border-gray-600 text-sm transition-all"
              >
                <SkipForward size={14} /> Skip
              </button>
              <button
                onClick={onFlag}
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-lg border text-sm transition-all ${
                  isFlagged ? "bg-amber-900/30 border-amber-700 text-amber-400" : "bg-gray-800 border-gray-700 text-gray-400 hover:text-gray-300 hover:border-gray-600"
                }`}
              >
        ...
        {submitted && (
          <div className="space-y-3 mt-4">
            <div className={`p-4 rounded-lg border ${selectedOption === question.correct ? "bg-green-900/20 border-green-800" : "bg-red-900/20 border-red-800"}`}>
              <div className="flex items-center gap-2 mb-2">
                {selectedOption === question.correct
                  ? <><CheckCircle size={18} className="text-green-400" /><span className="font-semibold text-green-400">Correct!</span></>
            ...
            <div className="p-4 rounded-lg border border-blue-800 bg-blue-900/15">
              <div className="flex items-center gap-2 mb-2">
            ...
            {question.proTip && (
              <div className="p-4 rounded-lg border border-amber-800 bg-amber-900/15">
                <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex gap-3">
              <button
                onClick={onFlag}
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-lg border text-sm transition-all ${
                  isFlagged ? "bg-amber-900/30 border-amber-700 text-amber-400" : "bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-600"
                }`}
              >
              ...
              <button
                onClick={onAnswer}
                className="flex-1 py-2.5 rounded-lg bg-blue-600 text-white font-medium text-sm hover:bg-blue-500 transition-all flex items-center justify-center gap-2"
              >
...
function ResultsScreen({ answers, questions, totalTime, onRetryMissed, onRetryWeak, onRestart }) {
  ...
  answers.forEach((a, i) => {
    ...
    if (!subtopicStats[st]) subtopicStats[st] = { correct: 0, total: 0 };
    subtopicStats[st].total++;
    if (a.correct) subtopicStats[st].correct++;
  });

  // Confidence analysis
  const luckyGuesses = answers.map((a, i) => ({ ...a, idx: i })).filter(a => a.correct && a.confidence === 0);
  const overconfidentMisses = answers.map((a, i) => ({ ...a, idx: i })).filter(a => !a.correct && a.confidence === 2);
  const missed = answers.map((a, i) => ({ ...a, idx: i })).filter(a => !a.correct);
  const flagged = answers.map((a, i) => ({ ...a, idx: i })).filter(a => a.flagged);

  const weakSubtopics = Object.entries(subtopicStats).filter(([_, s]) => (s.correct / s.total) < 0.7).map(([name]) => name);
  const hasWeakSubtopics = weakSubtopics.length > 0;
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">{correct}/{questions.length} correct · {mins}m {secs}s total</p>
        </div>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-4">
          ...
          <div className="space-y-3">
            {Object.entries(subtopicStats).map(([name, stats]) => {
              ...
              const color = p >= 80 ? "bg-green-500" : p >= 60 ? "bg-amber-500" : "bg-red-500";
              return (
                <div key={name}>
        ...
        {(luckyGuesses.length > 0 || overconfidentMisses.length > 0) && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-4">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
              <Brain size={16} /> Confidence Analysis
            </h3>
            {luckyGuesses.length > 0 && (
              <div className="mb-4">
                <p className="text-sm text-amber-400 font-medium mb-2">Lucky Guesses ({luckyGuesses.length}) — hidden weak spots</p>
                <div className="space-y-1">
                  {luckyGuesses.map(a => (
                    <p key={a.idx} className="text-xs text-gray-400 pl-3 border-l-2 border-amber-800">
                      Q{a.idx + 1}: {questions[a.idx].subtopic}
                    </p>
                  ))}
                </div>
              </div>
            ...
            {overconfidentMisses.length > 0 && (
              <div>
                <p className="text-sm text-red-400 font-medium mb-2">Overconfident Misses ({overconfidentMisses.length}) — dangerous misconceptions</p>
                <div className="space-y-1">
                  {overconfidentMisses.map(a => (
                    <p key={a.idx} className="text-xs text-gray-400 pl-3 border-l-2 border-red-800">
                      Q{a.idx + 1}: {questions[a.idx].subtopic}
                    </p>
                  ))}
                </div>
        ...
        {missed.length > 0 && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 mb-4 overflow-hidden">
            <button onClick={() => setShowSection(showSection === "missed" ? null : "missed")} className="w-full flex items-center justify-between p-4 hover:bg-gray-800/50 transition-all">
              <span className="text-sm font-semibold text-red-400">Incorrect Questions ({missed.length})</span>
              <ChevronRight size={16} className={`text-gray-500 transition-transform ${showSection === "missed" ? "rotate-90" : ""}`} />
            </button>
            {showSection === "missed" && (
              <div className="px-4 pb-4 space-y-4">
                {missed.map(a => {
                  ...
                  return (
                    <div key={a.idx} className="p-4 rounded-lg border border-gray-800 bg-gray-800/30">
                      <p className="text-xs text-gray-500 mb-1">Q{a.idx + 1} · {q.subtopic}</p>
        ...
        {flagged.length > 0 && (
          <div className="bg-gray-900 rounded-xl border border-gray-800 mb-4 overflow-hidden">
            <button onClick={() => setShowSection(showSection === "flagged" ? null : "flagged")} className="w-full flex items-center justify-between p-4 hover:bg-gray-800/50 transition-all">
              <span className="text-sm font-semibold text-amber-400 flex items-center gap-2"><Flag size={14} /> Flagged Questions ({flagged.length})</span>
              <ChevronRight size={16} className={`text-gray-500 transition-transform ${showSection === "flagged" ? "rotate-90" : ""}`} />
            </button>
            {showSection === "flagged" && (
              <div className="px-4 pb-4 space-y-4">
                {flagged.map(a => {
        ...
        <div className="space-y-3 mt-6">
          {missed.length > 0 && (
            <button onClick={onRetryMissed} className="w-full py-3 rounded-xl bg-red-900/30 border border-red-800 text-red-400 font-medium hover:bg-red-900/50 transition-all flex items-center justify-center gap-2">
              <RotateCcw size={16} /> Retry Missed Questions ({missed.length})
            </button>
          )}
          {hasWeakSubtopics && (
            <button onClick={onRetryWeak} className="w-full py-3 rounded-xl bg-amber-900/30 border border-amber-800 text-amber-400 font-medium hover:bg-amber-900/50 transition-all flex items-center justify-center gap-2">
              <AlertTriangle size={16} /> Retry Weak Subtopics ({weakSubtopics.length})
            </button>
          )}
          <button onClick={onRestart} className="w-full py-3 rounded-xl bg-gray-800 border border-gray-700 text-gray-300 font-medium hover:bg-gray-700 transition-all flex items-center justify-center gap-2">
...
export default function APIDesignQuiz({ quizSlug = 'api-design' }) {
  ...
  // Handle timeout
  useEffect(() => {
    if (timeLeft === 0 && !submitted && screen === "quiz") {
      setSubmitted(true);
  ...
  function startQuiz(mode, customOrder = null) {
    let order;
    if (customOrder) {
      order = customOrder;
  ...
  function handleAnswer() {
    ...
    if (currentIdx + 1 < questionOrder.length) {
      ...
      // Add skipped questions back
      const newOrder = [...questionOrder, ...skippedQueue];
  ...
  function handleSkip() {
    ...
    if (currentIdx + 1 < questionOrder.length) {
      ...
      setSubmitted(false);
      startTimer();
    } else if (skippedQueue.length > 0 || true) {
      // All regular questions done, but we have skipped ones. We'll append them.
      const updatedSkipped = [...skippedQueue, qIdx];
      const newOrder = [...questionOrder, ...updatedSkipped];
      setQuestionOrder(newOrder);
      setSkippedQueue([]);
      setCurrentIdx(currentIdx + 1);
      setSelectedOption(null);
  ...
  function handleRetryMissed() {
    const missedIndices = answers.filter(a => !a.correct).map(a => a.questionIdx);
    startQuiz("shuffled", shuffleArray(missedIndices));
  }

  function handleRetryWeak() {
    const subtopicStats = {};
    answers.forEach((a, i) => {
      const st = activeQuestions[i]?.subtopic || QUESTIONS[a.questionIdx].subtopic;
      if (!subtopicStats[st]) subtopicStats[st] = { correct: 0, total: 0 };
      subtopicStats[st].total++;
      if (a.correct) subtopicStats[st].correct++;
    });
    const weakNames = Object.entries(subtopicStats).filter(([_, s]) => (s.correct / s.total) < 0.7).map(([name]) => name);
    const weakIndices = QUESTIONS.map((q, i) => weakNames.includes(q.subtopic) ? i : -1).filter(i => i >= 0);
    startQuiz("shuffled", shuffleArray(weakIndices));
  }
  ...
  if (screen === "results") {
    return (
      <ResultsScreen
        answers={answers}
        questions={answers.map(a => QUESTIONS[a.questionIdx])}
  ...
  const currentQ = QUESTIONS[questionOrder[currentIdx]];
  ...
  return (
    <QuestionScreen
      ...
      onSkip={handleSkip}
      onFlag={handleFlag}
      isFlagged={flaggedSet.has(questionOrder[currentIdx])}
      timeLeft={timeLeft}
