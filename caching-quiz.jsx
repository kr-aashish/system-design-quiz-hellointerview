...
export const QUESTIONS = [
  {
    ...
    "options": [
      ...
      "The connection pool is undersized for peak traffic. Increase it to 5,000 connections to handle the burst traffic during cold start, and the cold start problem resolves itself naturally as the cache populates with user profiles through normal read operations.",
  ...
  },
  {
    ...
    "options": [
      "Switch to write-through caching so every new product is automatically cached at creation time, eliminating the first-view latency penalty entirely and ensuring no user ever experiences a slow cache miss on any product page regardless of how recently it was added to the catalog.",
      ...
      "Pre-populate every new product into the cache on creation using a background job that fires on the product creation event. This eliminates the cold read entirely with no meaningful trade-offs since the background job is asynchronous.",
      "The first-view penalty doesn't matter because CDN caching will handle new product pages before they ever reach the application cache layer, so the latency for the first viewer is absorbed by the edge server."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The database handles the load fine since it was designed and provisioned for the full traffic volume. The only impact is increased latency from 1ms to 50ms per query, which is noticeable but not catastrophic.",
      ...
      "The application servers crash from the increased memory usage of holding thousands of open database connections simultaneously. Use connection pooling with a bounded pool size to prevent this resource exhaustion.",
      "The Redis failover to a replica happens within 1-2 seconds via Redis Sentinel, so the traffic spike hitting the database is negligible and transient. Just configure Redis Sentinel for automatic failover and the problem is solved at the infrastructure layer."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      ...
      "CDN latency improvement is marginal (maybe 50ms savings) because images still need to be fetched from the origin on every request since CDN cache hit rates for user-generated content are typically low. The main benefit is DDoS protection at the edge, not latency reduction.",
      "CDN only helps for the first request from each edge server location. Subsequent requests use browser caching regardless of CDN presence, so the improvement is only for cache-cold users who haven't visited the page before — repeat visitors already have the images locally cached in their browser's HTTP cache."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "CDN caching is appropriate for both static images and dynamic API responses. Modern CDNs like Cloudflare support caching personalized API responses using Vary headers, edge-side includes (ESI), and edge compute functions, making them equally effective for dynamic content as they are for static assets.",
      ...
      "CDN should be used for neither type of content. API responses change too frequently for edge caching to provide meaningful hit rates, and images should be served directly from object storage like S3 which already has built-in global edge distribution through transfer acceleration.",
      "CDN is only useful for images over 1MB in size where the bandwidth savings justify the edge caching overhead. Profile thumbnails at 50-100KB are too small to benefit from edge caching since the transfer time savings are negligible compared to the connection setup cost."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Write-through guarantees perfect consistency with absolutely no trade-offs as long as both the cache and database are available. It only breaks down when Redis becomes unavailable, at which point writes fall back directly to the database.",
      ...
      "The trade-off is cache pollution — write-through caches every written value even if it's never read again, wasting precious Redis memory on entries that may never serve a cache hit. But consistency between cache and database is fully guaranteed because writes are synchronous.",
      "Both B and C are valid trade-offs of write-through that apply simultaneously. Slower writes from dual synchronous operations AND cache pollution from caching unread values. But consistency is still fully guaranteed because both the cache and database are updated atomically in the same synchronous operation."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      ...
      "Write-through is fine for this use case because it ensures all events are durably stored in both the cache and database before acknowledging. The cache simply acts as a write buffer that provides fast subsequent reads of recent events.",
      "The only issue is that Redis might not handle 100K writes/sec at sustained throughput. If you horizontally scale Redis with clustering and add more shards, write-through works perfectly well here because the fundamentals of the pattern are sound.",
      "Write-through fails because logging events are immutable and never updated after initial creation, so caching them provides no benefit for subsequent reads. But the write latency overhead is acceptable since logging pipelines are asynchronous."
    ],
    "correctIndex": 0,
  ...
  },
  {
    ...
    "options": [
      "No data is lost because Redis persists all writes to its AOF (append-only file) before acknowledging them to the application. The flush to the downstream database resumes automatically after Redis restarts and replays from its AOF log.",
      ...
      "Only the current batch being flushed at the moment of the crash is lost. All previously acknowledged writes are safe because Redis uses write-ahead logging that syncs to disk before returning success to the application.",
      "The entire cache is lost including all unflushed data, but this level of data loss is never acceptable in any production environment. Write-behind should only be used in development and staging environments where data durability is not a concern."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The reasoning is actually correct. As long as reads always go through the cache first and never bypass it to hit the database directly, the data is consistent from the application's perspective because the cache always has the latest write.",
      ...
      "The issue is only performance — write-behind batching creates latency spikes during flush operations that can temporarily saturate the database connection pool and affect read performance for all queries, not just the flushed data.",
      "Write-behind fails because the cache has limited memory capacity. Once Redis fills up with unflushed writes, new writes are rejected until the flush completes, creating application-level backpressure that manifests as timeouts and errors on the write path."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Bandwidth — uploading 3 runs worth of GPS data over WiFi takes too long and the transfer might timeout before completing, especially if each run contains thousands of latitude/longitude coordinate pairs recorded at one-second intervals over a 30-minute run.",
      ...
      "Authentication — the session token stored locally on the device may have expired during the offline period, preventing the sync entirely until the user re-authenticates through the login flow, which disrupts the seamless background sync experience.",
      "Data ordering — the 3 runs must be inserted in chronological order on the server or the user's cumulative statistics (weekly mileage, streak tracking) will be calculated incorrectly, and reordering after insertion would require expensive recalculation of all derived metrics."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "All 20 instances see the change immediately because in-process caches use shared memory across the load balancer, which broadcasts invalidation events to all connected application instances via its internal communication channel.",
      ...
      "The feature flag change propagates within 1-2 seconds via the load balancer's built-in cache invalidation broadcast mechanism, which pushes configuration updates to all registered backend instances through a persistent WebSocket connection.",
      "In-process caching doesn't work for feature flags because they change too frequently for local caching to be effective. The content recommends in-process caching only for truly static configuration values that are set once at deployment time and never change during the application's runtime lifecycle."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "This is a bug — cache invalidation should be synchronous and atomic with the DB write. Fix it by using distributed transactions between Redis and the database to ensure both systems update in a single atomic operation, eliminating any window for stale data.",
      ...
      "The invalidation is failing silently due to a transient Redis connection issue. Check the Redis connection health, add retry logic with exponential backoff, and implement logging to detect silent invalidation failures in production.",
      "This only happens because you're invalidating the cache entry instead of updating it directly. Switch to updating the cache entry with the new value on every write to eliminate the stale window entirely, since the cache always reflects the latest write."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "All three features should use cache invalidation on writes for maximum data freshness. TTL-based approaches are too risky for any production system because they guarantee a staleness window that could serve incorrect data to users.",
      ...
      "Bank balance: cache-aside with 1-second TTL for near-real-time accuracy. News feed: CDN caching at edge locations for low latency globally. Analytics dashboard: write-behind with 5-minute flush interval to batch aggregate computations."
    ],
    "correctIndex": 2,
  ...
  },
  {
    ...
    "options": [
      "The reasoning is correct. Eventual consistency means any delay is acceptable by definition, and 24 hours is just a longer eventual consistency window that will self-correct when the TTL expires and the cache refreshes from the database.",
      "The candidate correctly applies eventual consistency but should use write-behind caching instead of TTL-based expiration for better write performance, since write-behind batches the database updates and avoids the dual-write overhead.",
      ...
      "The 24-hour TTL is fine for prices, but the candidate should also add a CDN layer in front of the application cache to reduce the staleness window by serving fresher data from edge locations closer to the user."
    ],
    "correctIndex": 2,
  ...
  },
  {
    ...
    "options": [
      "Write-through only works with Redis as the caching layer, not Memcached, so it's not a general solution for all caching architectures and shouldn't be recommended without confirming the cache technology.",
      ...
      "Write-through is too slow for production workloads because it requires two synchronous writes to complete before acknowledging, but the consistency between cache and database is fully guaranteed as long as both writes succeed.",
      "Write-through solves consistency for writes but not for reads — cached data can still expire via TTL and the subsequent cache miss read might fetch stale data if replication lag exists between the primary database and any read replicas."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The database handles it fine because modern databases can process 5,000 concurrent queries in parallel using connection pooling and query optimization. The 200ms query time means all results return within 250ms with minimal overhead.",
      ...
      "The TTL expiration is automatically staggered by the cache's internal clock jitter, so requests naturally spread out over several seconds rather than all expiring at the exact same millisecond. No additional mitigation is needed.",
  ...
  },
  {
    ...
    "options": [
      "Cache warming doesn't work when the cache server has insufficient memory to hold all the warmed entries simultaneously, because the warming process competes with normal cache operations for the limited memory pool.",
      ...
      "Cache warming fails when the popular keys change frequently and unpredictably, because the warming process can't keep up with the rapidly shifting popularity patterns and ends up preloading keys that are already cold by the time they're cached.",
      "Cache warming is always effective regardless of the expiration strategy being used. The candidate's answer is correct — proactively refreshing popular keys before they expire prevents stampedes in all scenarios."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      ...
      "Probabilistic early expiration works by setting the TTL to infinity and relying on LRU eviction to naturally stagger removals as memory pressure increases. The trade-off is unpredictable cache sizes since entries live indefinitely until evicted.",
      "It randomly delays cache misses after the TTL expiration point so that requests spread out over a few seconds instead of all hitting the database at the exact same moment. The trade-off is higher worst-case latency for some users who experience the delayed miss."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      ...
      "Rate limit requests for Taylor Swift's profile to 100K/sec and serve users who exceed the rate limit a cached static version of her profile, isolating the hot key from excessive read pressure while still serving acceptable content.",
  ...
  },
  {
    ...
    "options": [
      "No benefit — the replicated Redis keys across 10 shards already handle the load effectively. In-process caching just wastes application server memory on data that's already available at sub-millisecond latency from the local Redis shard.",
      ...
      "In-process caching helps only if Redis goes down entirely. It's a failover/fallback layer for availability during Redis outages, not a performance optimization for the normal operating path when Redis is healthy and responsive.",
      "In-process caching is faster but creates a distributed cache coherence problem across the application server fleet that's fundamentally unsolvable without consensus protocols like Raft or Paxos to coordinate updates between instances."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      ...
      "1. Replicate the hot key across multiple Redis shards first (highest throughput gain from distributing the concentrated load). 2. Add in-process caching on each app server. 3. Add rate limiting on suspicious traffic patterns. 4. Investigate root cause.",
      "1. Rate limit immediately to stop the bleeding and prevent the shard from crashing. 2. Investigate the root cause to determine if this is legitimate traffic or a bot attack. 3. Replicate the hot key across shards. 4. Add in-process caching as a long-term optimization.",
      "1. Scale up the Redis shard vertically (more CPU/memory) to handle the immediate load. 2. Add read replicas for the shard. 3. Add in-process caching on application servers. 4. Implement rate limiting on the hot key."
    ],
    "correctIndex": 0,
  ...
  },
  {
    ...
    "options": [
      "Nothing — Redis automatically prioritizes smaller objects over larger ones to maximize the overall cache hit ratio, using a size-aware eviction policy that considers both recency and object size.",
      ...
      "Redis refuses to cache objects larger than 1MB by default due to its internal memory allocation constraints, so the 5MB PDFs are silently rejected and the user profiles remain unaffected in the cache.",
      "The PDFs are cached but immediately evicted on the very next eviction cycle because LRU detects from their access pattern metadata that they won't be accessed again soon, so they are deprioritized below frequently accessed entries."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The candidate is correct — LRU makes TTL unnecessary because stale entries will eventually be evicted when memory pressure increases and newer entries need the space, which effectively refreshes the data naturally.",
      ...
      "Without TTL, the cache grows unbounded because LRU only activates when the configured memory limit is hit. The cache will consume all available server memory before LRU starts evicting, which can starve other processes running on the same host.",
      "The only issue is that session tokens won't expire on schedule, creating a security vulnerability where sessions remain valid indefinitely. Other data types like user profiles and product listings are fine without TTL because they don't have security-sensitive expiration requirements."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "CDNs have significantly more memory than a typical Redis deployment, so they can handle the additional complexity of read-through caching without the memory pressure constraints that make read-through impractical for Redis.",
      ...
      "Read-through requires consistent hashing for routing cache misses to the correct origin, which CDNs support natively through their edge routing infrastructure but Redis does not implement without additional client-side libraries.",
      "CDNs only cache static files like images and CSS which are simple to read-through because they're served from a filesystem path. Redis caches complex query results that require application logic to reconstruct, making automatic read-through impractical for dynamic data."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "FIFO, because it evicts the oldest entries first, which naturally removes videos that haven't been popular recently since older entries were cached earlier. It's simple to implement with a queue data structure and has minimal computational overhead.",
      "LRU, because it evicts the least recently accessed video, which handles trending videos well since they're accessed frequently while trending and naturally evicted when the trend ends and access drops off. LRU's simplicity and predictability make it the safest default choice for most caching workloads.",
      ...
      "TTL-only with no eviction policy. Set a 7-day TTL on all videos and let them expire naturally, which avoids the complexity of maintaining access counters or recency lists while still ensuring the cache refreshes regularly and doesn't serve indefinitely stale content to viewers."
    ],
    "correctIndex": 2,
  ...
  },
  {
    ...
    "options": [
      "Set all three to 5 minutes for simplicity and operational consistency. Uniform TTLs are easier to manage, monitor, and debug because there's only one timeout value to reason about across the entire system.",
      ...
      "Sessions: no TTL (let the application manage expiration through explicit deletion on logout or inactivity timeout). Profiles: 24 hours (since they change weekly on average, a full-day TTL captures most changes). Leaderboard: 1 minute (for near-real-time updates that keep competitive players engaged).",
      "Sessions: 30 minutes (matching the session timeout). Profiles: 7 days (exactly matching the average change frequency so the cache refreshes approximately when changes occur). Leaderboard: no TTL (invalidate on write from the batch job since the job knows exactly when data changes)."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "A 5-second TTL works fine and is the optimal trade-off. The cache still provides significant latency reduction (1ms cached vs 50ms from DB) and the data is nearly real-time, giving users a fresh experience during viral events without adding architectural complexity.",
      ...
      "The 5-second TTL causes the cache to use too much memory due to the overhead of frequent key creation, expiration tracking, and deletion cycles. Keep the 60-second TTL and accept the staleness as an acceptable trade-off for overall cache stability and reduced operational complexity.",
      "The problem is that a 5-second TTL makes the cache nearly indistinguishable from a direct database read in terms of freshness. Remove caching entirely during viral events and let the database handle the load directly since the data changes too fast for caching to provide meaningful benefit."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Nothing is wrong — caching is always needed for read-heavy systems and a URL shortener is inherently read-heavy, so proposing Redis early demonstrates good architectural instincts.",
      ...
      "The issue is that you proposed Redis specifically without evaluating alternatives. You should have said 'an external cache' without naming a specific technology, letting the discussion evolve naturally to the technology choice.",
      "URL shorteners don't need caching because the database query (a single key lookup by short URL hash) is already fast enough at single-digit milliseconds. The interviewer is telling you caching is the wrong solution for this particular system."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The TTL is too long for a real-time chat application. Chat messages should have a 5-minute TTL at most for real-time freshness, since conversations move fast and users expect to see the latest messages without any perceptible delay from stale cache entries that were written minutes or hours ago.",
      "Redis can't handle the write volume. A chat app with millions of active users generates too many messages per second for a single Redis cluster to ingest, and the write throughput required for caching every individual message would overwhelm the cache layer before the database even becomes a bottleneck.",
      ...
      "Chat messages are too small in size to benefit meaningfully from caching. The latency difference between a cached and uncached small text message of a few hundred bytes is negligible at sub-millisecond versus single-digit milliseconds, and the operational overhead of cache management outweighs any marginal performance gain for such small payloads."
    ],
    "correctIndex": 2,
  ...
  },
  {
    ...
    "options": [
      "1. Choose Redis as the cache technology. 2. Explain the cache-aside pattern for read operations. 3. Set LRU eviction policy. 4. Address consistency with invalidation on writes. 5. Identify what data to cache.",
      ...
      "1. Address downsides and failure modes first to demonstrate awareness and maturity. 2. Choose the cache architecture. 3. Decide what data to cache. 4. Set the eviction policy and TTL. 5. Circle back to identify the bottleneck.",
      "1. Propose CDN caching for static assets first. 2. Add Redis for dynamic data caching. 3. Add in-process caching for the hottest keys. 4. Explain the original bottleneck that motivated adding caching layers to the architecture."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Discuss Redis running out of memory, keys not being found on cache miss, and network latency to Redis increasing under load — these are the most common production caching issues that any senior engineer should address.",
      ...
      "List every possible failure mode exhaustively: Redis down, network partition, key not found, TTL too short, TTL too long, LRU evicting important data, hot keys, cold start, stale data, memory fragmentation, and more. Comprehensive coverage shows thorough understanding.",
      "Focus solely on the thundering herd problem since it's the most technically complex failure mode. Spend 5 minutes walking through the single-flight implementation details, including the mutex acquisition pattern and the request queue management."
    ],
    "correctIndex": 1,
