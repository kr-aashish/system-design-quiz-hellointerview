...
const QUESTIONS = [
  ...
  },
  {
    ...
    question:
      "You propose cache-aside for a product catalog service. Your interviewer says: 'Cache-aside only caches data that's been requested. What about new products that haven't been viewed yet — won't the first viewer always get slow response times?' What's the strongest response?",
    options: [
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "Your cache-aside implementation invalidates the cache entry on every write, then the next read repopulates it from the database. A colleague points out that there's a subtle race condition in this seemingly safe pattern. What's the issue?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "Cache-aside has two subtle consistency gaps. First, a concurrent read between cache invalidation and DB commit can re-cache stale data. Second, if the cache invalidation call fails (e.g., Redis network blip) but the DB write succeeds, you have permanently stale cached data until TTL expiration. This is why the content states 'there is no perfect solution' for cache consistency — you must choose a strategy based on freshness requirements. Short TTLs provide a safety net for both failure modes.",
    interviewScript:
      "In your interview, say: 'Cache-aside has a known race condition: a read between cache invalidation and DB commit can re-cache stale data. There's also the risk of invalidation failure — if the Redis call fails but the DB write succeeds, stale data persists. Short TTLs act as a safety net for both cases. Perfect consistency with cache-aside isn't achievable without distributed transactions, which is rarely worth the complexity.'",
    proTip:
      "Mentioning the invalidation race condition unprompted shows deep understanding. Most candidates only think about the happy path of cache-aside.",
  },
  {
    id: "q4",
    ...
    question:
      "You're designing a session store for a web app with 5M daily active users. Each session is ~2KB and sessions expire after 30 minutes of inactivity. You're choosing between Redis and Memcached. Given these constraints, which is the better choice and why?",
    options: [
  ...
  },
  {
    ...
    question:
      "Your application uses Redis as an external cache with cache-aside. Redis goes down during peak traffic (100K requests/sec). Your application falls back to hitting the database directly. What is the most dangerous failure cascade and how should you design against it?",
    options: [
  ...
  },
  {
    ...
    question:
      "Your image-heavy social media app serves 500M image requests per day. Your origin server is in Virginia. 40% of traffic comes from India (250-300ms RTT to origin). You're evaluating whether to add a CDN. What's the latency improvement and what's the primary operational benefit?",
    options: [
  ...
  },
  {
    ...
    question:
      "You're designing a system that serves both static profile images and personalized API responses (user feeds). A junior engineer suggests putting both behind a CDN. When should you push back on CDN caching and why?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly states: 'the safest time to introduce a CDN is when your system serves static media at scale.' While modern CDNs can cache API responses, personalized feeds have near-zero cache hit rates because every user sees different content. A CDN miss adds latency (extra hop to edge, then to origin) rather than reducing it. Start with static media where cache hit rates are high, then expand only if needed.",
    interviewScript:
      "In your interview, say: 'I'd CDN-cache the profile images — static content with high cache hit rates is the sweet spot. But personalized feeds are a poor fit because each user sees different data, so the cache hit rate approaches zero. A CDN miss actually adds latency from the extra hop. I'd serve feeds directly from the application cache or database.'",
    proTip:
      "In interviews, distinguish between what CDNs CAN do and what they SHOULD do. Saying 'modern CDNs can cache API responses' is true but shows shallow understanding if you don't explain when it's counterproductive.",
  },
  {
    id: "q8",
  ...
  },
  {
    ...
    question:
      "A candidate proposes write-through caching for a logging and analytics pipeline that ingests 100K events per second. Events are written once and rarely read within the first hour. What is the most critical flaw in this design?",
    options: [
  ...
  },
  {
    ...
    style: "Critical Failure Modes",
    question:
      "You implement write-behind caching for a metrics pipeline. Writes go to Redis, which batches and flushes to the database every 5 seconds. During a flush, the Redis instance crashes. What data is lost, and for what types of workloads is this an acceptable risk?",
    options: [
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "A candidate claims write-behind caching gives them 'the best of both worlds: fast writes AND strong consistency, because the data is always in the cache for reads and eventually makes it to the database.' What's wrong with this reasoning?",
    options: [
    ...
    interviewScript:
      "In your interview, say: 'Write-behind is an eventual consistency pattern, not a strong consistency one. There's a window where data exists only in the cache. If the cache fails, that data is gone. And any system reading from the database directly — reporting, analytics, other microservices — sees stale data until the flush completes.'",
    proTip:
      "Interviewers love to test if you can spot the consistency model mismatch. Whenever someone says 'eventually consistent' and 'strong consistency' in the same breath, that's a red flag to probe.",
  },
  {
    id: "q12",
    ...
    question:
      "Your mobile app caches user run data locally for offline use and syncs when connectivity returns (like Strava). A user completes 3 runs offline, then connects to WiFi. Meanwhile, they also edited their profile on the web app. When the mobile app syncs, what's the most architecturally challenging problem and why?",
    options: [
  ...
  },
  {
    ...
    question:
      "Your service has 20 application server instances behind a load balancer. You add an in-process cache for feature flags that refreshes every 60 seconds. A product manager toggles a feature flag off due to a critical bug. What happens in the worst case, and what constraint from the content makes this acceptable?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly states in-process caching limitations: 'Each instance of your application has its own cache, so cached data is not shared across servers. If one instance updates or invalidates a cached value, the others will not know.' With 20 instances refreshing independently on 60-second intervals, a flag toggle takes up to 60 seconds to propagate to all instances. The content lists feature flags as an appropriate use case because they're 'small, frequently accessed values that rarely change.'",
    interviewScript:
      "In your interview, say: 'In-process caching means each of my 20 instances has its own copy. When a feature flag is toggled, instances won't know until their next refresh — up to 60 seconds. For most flag changes, this is fine. For critical bug mitigation, I might add a pub/sub notification channel to trigger immediate cache invalidation across instances, while keeping the 60-second refresh as the normal path.'",
    proTip:
      "Mention in-process caching as an optimization AFTER proposing an external cache. The content says: 'mention this only as an optimization layer after you have already introduced an external cache.'",
  },
  {
    id: "q14",
    ...
    correctIndex: 1,
    explanation:
      "The content explains this exact scenario: 'if a user updates their profile picture, the new value is written to the database but the old value might still be in the cache. Other users may see the outdated profile picture until the cache eventually refreshes.' The recommended strategies are invalidation on writes (which you're doing) combined with short TTLs as a safety net. The content states: 'there is no perfect solution. You choose a strategy based on how fresh the data must be.'",
    interviewScript:
      "In your interview, say: 'This is expected eventual consistency, not a bug. With cache-aside, there's a brief window where stale data can be served. For profile pictures, a 2-second delay is acceptable. I use cache invalidation on writes as the primary mechanism and short TTLs as a safety net. If we needed true real-time consistency, we'd pay a higher complexity cost with write-through, but for social profiles that's overkill.'",
    proTip: null,
  },
  {
    id: "q15",
    ...
    style: "Decision Framework Application",
    question:
      "You're designing three different features: (1) a user's bank balance display, (2) a social media news feed, (3) an analytics dashboard showing daily active users. For each, you need to choose a cache consistency strategy. Which combination correctly applies the content's guidance?",
    options: [
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "A candidate proposes write-through caching and claims it 'solves cache consistency because the cache and database are always updated together synchronously.' The content identifies a specific problem that persists even with write-through. What is it?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly warns about this: 'Write-through still suffers from the dual-write problem. If the cache update succeeds but the database write fails, or vice versa, the systems can end up inconsistent. You need retry logic, error handling, or eventually accept that perfect consistency is difficult without distributed transactions.' Write-through improves consistency but doesn't guarantee it.",
    interviewScript:
      "In your interview, say: 'Write-through improves consistency but doesn't guarantee it. The dual-write problem persists — if the cache write succeeds but the DB write fails, you have an inconsistent state. True consistency requires distributed transactions, which add significant complexity. In practice, I'd add retry logic and idempotent writes as a pragmatic solution.'",
    proTip:
      "This is a favorite interviewer gotcha. If a candidate says any caching pattern 'guarantees consistency,' probe the dual-write problem. The only guarantee comes from distributed transactions, and those have their own costs.",
  },
  {
    id: "q18",
    ...
    style: "Failure Analysis",
    question:
      "Your homepage feed cache has a 60-second TTL. At exactly the expiration moment, 5,000 concurrent requests arrive. The feed query takes 200ms and requires joining 3 tables. Without any stampede protection, what happens to your database and what is the most effective mitigation?",
    options: [
  ...
  },
  {
    ...
    style: "Gotcha/Trap",
    question:
      "A candidate proposes cache warming (proactively refreshing popular keys before they expire) as the solution to cache stampedes. Under what specific condition does cache warming NOT prevent stampedes, according to the content?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content explicitly states this nuance: 'Cache warming: Refresh popular keys proactively before they expire. This only helps when using TTL-based expiration. If you invalidate cache on writes instead, warming does not prevent stampedes.' This is because write-based invalidation happens at unpredictable times — you can't proactively refresh a key that was just invalidated by a write. You need request coalescing for write-invalidation scenarios.",
    interviewScript:
      "In your interview, say: 'Cache warming helps with TTL-based expiration because you can schedule refreshes before expiry. But if I'm invalidating on writes — deleting the cache entry when data changes — warming doesn't help because the invalidation is unpredictable. For write-invalidation patterns, I need request coalescing instead.'",
    proTip:
      "This distinction is rarely mentioned by candidates but shows sophisticated understanding. Always pair your stampede mitigation strategy with your invalidation strategy.",
  },
  {
    id: "q20",
    subtopic: "Cache Stampede (Thundering Herd)",
    ...
    question:
      "You use TTL-based expiration with a 60-second TTL on trending post feeds. You want to prevent stampedes without the complexity of request coalescing. The content mentions probabilistic early expiration. How does this interact with TTL, and what's the trade-off?",
    options: [
  ...
  },
  {
    ...
    style: "Scenario-Based Trade-offs",
    question:
      "You're building a Twitter-like system. Taylor Swift tweets during the Super Bowl, and her profile key (user:taylorswift) receives 5 million requests per second. Your Redis cluster has 10 shards, but this key maps to a single shard. That shard is overwhelmed while the other 9 are idle. What's the most effective mitigation?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content specifically uses this Taylor Swift example and recommends: 'Replicate hot keys: Store the same value on multiple cache nodes and load balance reads across them.' Adding more shards doesn't help because consistent hashing still maps the key to one shard. A dedicated instance helps but is reactive and doesn't scale to multiple hot keys. Replication across shards with read distribution is the standard solution.",
    interviewScript:
      "In your interview, say: 'This is a hot key problem. Adding shards doesn't help because the key still maps to one shard. I'd replicate the hot key across multiple nodes — store copies as user:taylorswift:1 through user:taylorswift:10 on different shards, and randomly route reads across them. This distributes the 5M requests across 10 nodes, bringing each to 500K/sec.'",
    proTip:
      "The Taylor Swift example is directly from the content. Use real-world analogies like this in interviews — it shows you've internalized the concepts beyond textbook definitions.",
  },
  {
    id: "q22",
    subtopic: "Hot Keys",
    style: "Cross-Subtopic Bridge",
    question:
      "Continuing the Taylor Swift hot key scenario: you've replicated the key across 10 Redis shards. A senior engineer suggests also adding an in-process cache for her profile on each application server. What additional benefit does this provide, and what's the risk?",
    options: [
  ...
  },
  {
    ...
    question:
      "You detect a hot key issue in production. The key receives 10x more traffic than any other and the Redis shard is at 95% CPU. In what order should you apply mitigations for maximum immediate impact with minimum risk?",
    options: [
  ...
  },
  {
    ...
    style: "Implementation-Specific Nuance",
    question:
      "Your Redis cache uses LRU eviction and has a 10GB memory limit. Your application caches both user profiles (accessed frequently, ~2KB each) and report PDFs (accessed once per month, ~5MB each). The cache is full. A batch job loads 100 report PDFs into the cache. What happens to cache performance and why?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "LRU evicts based solely on access recency. The batch job makes the PDFs 'recently accessed,' so LRU evicts older entries — the user profiles — regardless of their access frequency or usefulness. 100 PDFs at 5MB each = 500MB, which evicts roughly 250,000 2KB profiles. This is a classic LRU pathology: a batch scan pollutes the cache. This is why some systems use LFU or a combination of LRU + TTL.",
    interviewScript:
      "In your interview, say: 'This is a classic LRU pollution problem. The batch job makes infrequently-accessed PDFs appear recently used, evicting 250K frequently-accessed profiles. LRU doesn't consider access frequency, only recency. I'd mitigate this by either using a separate cache for reports, setting very short TTLs on report entries, or switching to an LFU-aware eviction policy for the main cache.'",
    proTip:
      "Cache pollution from batch jobs is a real production issue. Mentioning it shows operational experience beyond textbook knowledge.",
  },
  {
    id: "q25",
    subtopic: "LRU (Least Recently Used)",
    style: "Anti-Pattern Identification",
    question:
      "A candidate sets LRU eviction on their Redis cache but sets no TTL on any keys. They argue: 'LRU handles everything — it evicts entries when memory is full, so TTLs are redundant.' What critical problem does this create?",
    options: [
  ...
  },
  {
    ...
    style: "Cross-Subtopic Bridge",
    question:
      "The content states that CDNs are a form of read-through cache, yet recommends cache-aside over read-through for application-level caching with Redis. What architectural property makes read-through natural for CDNs but unnecessarily complex for Redis-based application caching?",
    options: [
  ...
  },
  {
    ...
    style: "Decision Framework Application",
    question:
      "You're choosing an eviction policy for a video streaming platform's cache. Some videos trend for 2-3 days then disappear, while others (classics) are consistently popular over months. A third category is new uploads that may or may not become popular. Which eviction policy best handles this mixed workload and why?",
    options: [
    ...
    correctIndex: 2,
    explanation:
      "The content specifically maps LFU to this use case: 'works well when certain keys are consistently popular over time, like trending videos or top playlists.' LFU's frequency tracking ensures classic videos (high cumulative access count) survive eviction while flash trends with lower total access get evicted first. FIFO is explicitly called out as problematic: 'it ignores usage patterns' and 'may evict items that are still hot.' LRU would evict classics during overnight low-traffic hours.",
    interviewScript:
      "In your interview, say: 'LFU is the best fit here. It tracks access frequency, so classic videos with thousands of cumulative views survive eviction even during overnight dips, while new uploads that don't gain traction are evicted first. FIFO would be dangerous — it ignores popularity entirely and could evict a classic. LRU works for trending content but might evict long-tail classics during quiet hours.'",
    proTip: null,
  },
  {
    id: "q28",
    ...
    style: "Estimation-Backed Reasoning",
    question:
      "You're setting TTLs for different cached data types in your system. Session tokens, user profiles, and a global leaderboard all need different TTLs. Given that your session timeout is 30 minutes, profiles change on average once per week, and the leaderboard updates every 5 minutes from a batch job, what's the optimal TTL strategy?",
    options: [
  ...
  },
  {
    ...
    style: "Interviewer Pushback",
    question:
      "You're 5 minutes into a system design interview for a URL shortener. You immediately say: 'We should add a Redis cache to reduce database load.' The interviewer responds: 'Why do we need caching?' What's the problem with your approach, according to the content?",
    options: [
  ...
  },
  {
    ...
    style: "Anti-Pattern Identification",
    question:
      "A candidate designing a real-time chat application proposes caching every message in Redis with a 24-hour TTL. They say: 'Messages are read frequently, so we should cache them all.' The content's guidance on 'what to cache' reveals why this is a flawed strategy. What's wrong?",
    options: [
  ...
  },
  {
    ...
    style: "Ordering/Sequencing",
    question:
      "The content provides a systematic framework for introducing caching in interviews. You've just identified that your database CPU is at 80% during peak hours from repeated identical queries. In what order should you present your caching strategy?",
    options: [
    ...
    correctIndex: 1,
    explanation:
      "The content provides this exact 5-step framework: (1) Identify the bottleneck, (2) Decide what to cache, (3) Choose your cache architecture, (4) Set an eviction policy, (5) Address the downsides. This order is deliberate — you establish the WHY before the HOW, choose what to cache before how to cache it, and address downsides last to show you understand the trade-offs after presenting the solution.",
    interviewScript:
      "In your interview, say: 'Let me walk through my caching strategy systematically. First, the bottleneck: DB CPU at 80% from repeated identical queries. Second, what to cache: [specific queries and key design]. Third, architecture: cache-aside with Redis. Fourth, eviction: LRU with 10-minute TTL. Fifth, failure handling: cache invalidation on writes, circuit breakers for Redis failure, and request coalescing for stampede prevention.'",
    proTip:
      "Following a clear, numbered framework in your interview shows structured thinking. Interviewers remember candidates who present systematically rather than stream-of-consciousness.",
  },
  {
    id: "q33",
    subtopic: "Interview Strategy",
    ...
    correctIndex: 1,
    explanation:
      "The content advises for staff-level: 'focus on the important but non-obvious scenarios rather than burning time on things the interviewer can already assume.' The three selected scenarios (invalidation race, failure cascade, thundering herd) are non-obvious and demonstrate deep understanding. Listing every failure mode wastes time on obvious ones. Focusing on just one misses the breadth expected at staff level.",
    interviewScript:
      "In your interview, say: 'Three failure scenarios I'd highlight. First, cache invalidation has a race condition — a read between invalidation and DB commit can re-cache stale data; short TTLs are my safety net. Second, Redis failure cascades to the database; I'd add circuit breakers and a small in-process cache as a last resort. Third, thundering herd on popular key expiration; I'd use request coalescing so only one request rebuilds the cache.'",
    proTip:
      "The content says: 'Don't list every possible problem. Pick one or two that are relevant.' For staff-level, pick the non-obvious ones. For SDE2, covering the basics thoroughly is fine.",
  },
];

const SUBTOPIC_TIERS = {
  Heavy: [
    { name: "Cache-Aside (Lazy Loading)", count: 3 },
    { name: "Cache Consistency", count: 4 },
    { name: "Cache Stampede (Thundering Herd)", count: 3 },
    { name: "Hot Keys", count: 3 },
    { name: "Interview Strategy", count: 4 },
  ],
  Medium: [
    { name: "External Caching (Redis/Memcached)", count: 2 },
    { name: "CDN (Content Delivery Network)", count: 2 },
    { name: "Write-Through Caching", count: 2 },
    { name: "Write-Behind (Write-Back) Caching", count: 2 },
    { name: "LRU (Least Recently Used)", count: 2 },
    { name: "TTL (Time To Live)", count: 2 },
  ],
  Thin: [
    { name: "Client-Side Caching", count: 1 },
    { name: "In-Process Caching", count: 1 },
    { name: "Read-Through Caching", count: 1 },
    { name: "LFU + FIFO (Clustered)", count: 1 },
  ],
};
...
export default function CachingQuiz() {
  ...
  const startRetry = useCallback((questionSubset) => {
    const qs = shuffleArray(questionSubset);
    setQuestions(qs);
    setCurrentIndex(0);
    setSelectedOption(null);
    setConfidence(null);
      ...
      <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-red-900/30 border border-red-700/50 text-red-400 px-3 py-1 rounded-full text-sm mb-4">
              <Target size={14} />
              FAANG SDE2 — Hard
            </div>
            <h1 className="text-4xl font-bold mb-2">Caching</h1>
            <p className="text-gray-400 text-lg mb-1">
              System Design Deep Dive
            </p>
            <p className="text-gray-500 text-sm">
              Cache architectures, eviction policies, consistency strategies,
              failure modes, and interview frameworks.
            </p>
          </div>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-8">
            ...
            {Object.entries(SUBTOPIC_TIERS).map(([tier, topics]) => (
              <div key={tier} className="mb-4 last:mb-0">
                ...
                >
                  {tier} ({topics.reduce((s, t) => s + t.count, 0)} questions)
                </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm text-gray-400">
              {showingSkipped && (
                <span className="text-amber-400 mr-2">[Skipped]</span>
              )}
  ...
  if (screen === "results") {
    ...
    const grade =
      score >= 90
        ? {
            label: "Staff-ready — you'd ace this topic",
            color: "text-green-400",
            bg: "bg-green-500/10 border-green-500/30",
          }
        : score >= 75
        ? {
            label: "Strong Senior — solid, minor gaps to close",
            color: "text-blue-400",
            bg: "bg-blue-500/10 border-blue-500/30",
          }
        : score >= 60
        ? {
            label: "SDE2-level — review the weak areas below",
            color: "text-amber-400",
            bg: "bg-amber-500/10 border-amber-500/30",
          }
        : {
            label: "Needs deep review — revisit the fundamentals",
            color: "text-red-400",
    ...
    QUESTIONS.forEach((q) => {
      if (!subtopicStats[q.subtopic])
        subtopicStats[q.subtopic] = { total: 0, correct: 0 };
    ...
    const overconfidentMisses = QUESTIONS.filter(
      (q) =>
        ...
        answers[q.id]?.confidence === "confident"
    );
