...
export const QUESTIONS = [
  {
    ...
    "options": [
      "Engineer X — the volume of expert exposure is higher, and the note-taking step converts passive viewing into active encoding because the act of paraphrasing into notes forces the same generative effort that whiteboard practice does. Forty hours of structured exposure plus written reformulation creates more durable encoding than fourteen attempted designs against an answer key.",
      ...
      "Engineer X — observing expert reasoning lets you internalize the *sequencing* of decisions (what to ask first, when to introduce sharding, how to defend trade-offs), and that sequencing transfers directly to interview performance. Engineer Y is producing answers without yet having seen enough expert moves to know which sequences are correct, so Y's attempts reinforce idiosyncratic patterns rather than expert ones.",
      "The two are roughly equivalent on retention but differ in transfer distance — Engineer X builds a broader mental library of expert-demonstrated patterns through high-volume observation, while Engineer Y builds deeper procedural fluency on a smaller set through generative practice. Under interview pressure the decisive variable is total hours invested rather than the input/output ratio of those hours, because both modalities converge on the same long-term retrieval strength once the volume crosses approximately 40 hours of deliberate engagement."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "They skipped the spaced-repetition phase of the practice loop — re-attempting the same three questions across multiple days is what accumulates enough exposures for unfamiliar terms (load balancer, cache, write-through) to become recognizable. Without that repetition, every term still feels foreign on attempt three because three exposures isn't enough to create durable encoding regardless of whether foundation reading was completed first.",
      ...
      "They skipped choosing a delivery framework. The framework provides structural scaffolding for the answer — without it, even fully-understood concepts can't be sequenced into a coherent design arc, so the candidate experiences the chaos of unstructured attempts as 'I don't recognize the terms' when the real issue is that the terms have no organizational slot in their answer structure, and the cognitive overload of simultaneously organizing and designing makes familiar terms feel unrecognizable.",
      "They skipped the mock interview step that should have preceded solo practice. Mock interviews are where unfamiliar vocabulary gets rehearsed against a live interlocutor who provides real-time correction — without that early calibration, terms that seemed clear in written form feel completely foreign when spoken under time pressure because the candidate has never mapped written concepts to the verbal register that interviews demand, creating a translation gap that solo practice cannot close."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Candidate B — organic exploration tracks the actual shape of the problem rather than a generic checklist, so the extended time on the data model reflects genuine architectural depth that the interviewer is looking for. The framework Candidate A used forces premature movement past important concerns just to hit a predetermined sequence; what looks like 'finishing deep dives' is really 'truncating the detailed modeling work the interviewer was waiting to evaluate,' since interviewers weight depth of reasoning over breadth of coverage.",
      ...
      "Candidate A — the visible framework signals to the interviewer that the candidate has internalized a recognized industry methodology, which is itself a strong senior signal independent of design quality. Without the visible structure, even a technically strong design reads as 'lucky improvisation' because the interviewer cannot distinguish intentional prioritization from accidental omission, so the framework's primary value is the credibility halo it confers on the work it produces, and its absence actively undermines otherwise good designs.",
      "Both reflect the case for a framework equally well — A uses an explicit framework while B uses an implicit one shaped by engineering experience, and under interview pressure both fall back to deeply internalized step ordering. The difference is presentation style rather than substance. Candidate B's extended data-model work may actually impress a senior interviewer who values depth, so the framework's value is purely as a communication aid that makes progress legible, not as a tool that changes the quality of the design itself."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "They're under-investing in answer-key absorption. Fifteen minutes is enough to skim an answer key but not enough to internalize the trade-offs and causal reasoning behind each architectural choice, so the candidate's subsequent attempt borrows the surface structure without the underlying reasoning. The corrected ordering keeps reading-first but stretches the answer-key phase to 30+ minutes of active annotation, with the attempt acting as verification that the absorbed reasoning has stuck rather than as the primary learning event.",
      ...
      "The cycle's structure is fine but the feedback calibration step is missing — mock interviews are the only mechanism that exposes whether 'I can follow this answer' has actually become 'I can produce this answer' under time pressure and social scrutiny. Adding two mocks per week to the existing read-then-attempt cycle would have caught the production gap weeks earlier, since the mock-interview signal is the only reliable way to calibrate self-evaluation against external reality and surface the illusion of competence that solo practice cannot detect.",
      "The re-read step is the cognitive failure — comparing your attempt to the answer key after just having read it 30 minutes earlier creates the illusion of consistency between the two because short-term memory still holds the structure, making even superficial matches feel like deep understanding. The corrected ordering eliminates the re-read entirely: read the answer once, attempt the design, and let a fresh evaluation a week later — ideally in mock-interview format — expose what actually stuck versus what was familiarity-bias masquerading as retained knowledge."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "This is high-leverage preparation — framework choice compounds across every subsequent practice attempt, so a week spent rigorously comparing options pays back across the next 50+ design attempts. The candidate is correctly identifying that getting the foundation right matters more than racking up early-stage volume against a suboptimal framework, since a poor framework creates systematic blind spots that practice alone won't correct, and switching frameworks mid-preparation is costlier than front-loading the decision.",
      ...
      "The candidate should synthesize their own custom framework by extracting the strongest step from each of the five they've read — a personalized sequence will fit their cognitive style better than any off-the-shelf framework, and the synthesis exercise itself deepens understanding of why each step exists. Once the custom framework is drafted and validated against two practice problems, practice attempts can begin with full conviction in a structure that matches how they naturally think through systems.",
      "The candidate should pivot to memorizing canonical answer keys for the top 20 most-asked design questions, which provides direct interview-day leverage without depending on any specific framework. Frameworks help organize unfamiliar problems, but for the standard interview canon (Twitter, Uber, chat system, URL shortener) the answer-key patterns are well-known enough that recall of specific architectural templates beats framework-driven derivation under time pressure, since the time saved on navigation can be redirected to deep dives."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Mock interviews replicate the time pressure and social-evaluative stress of the real interview far more faithfully than self-paced solo practice. The 45-minute clock combined with another human waiting for output produces a cognitive load profile that solo practice cannot reproduce, and that cognitive load is itself the differentiating factor between candidates who perform well in practice and those who perform well when stakes are high. Self-evaluation from a written answer key cannot replicate this load because the absence of social stakes fundamentally changes the cognitive environment.",
      ...
      "Mock interviews are the only setting in which the soft-skill components of the interview get rehearsed: hypothesis-statement framing, the precise language used to ask clarifying questions, real-time diagram narration, and the tonal calibration when admitting uncertainty on a technical point. Technical answer keys cover the design content but never the conversational scaffolding around it, and these soft skills are where mid-level candidates routinely lose points despite producing technically correct designs in their solo practice, because solo practice never exercises the verbal delivery layer.",
      "The mentor is over-indexing on a process step rather than the underlying signal it produces. Ten thoroughly-self-evaluated questions yields more genuine learning than two mocks plus eight rushed self-evaluations, because the depth of self-evaluation correlates more strongly with retention than the format of evaluation does. The candidate's discipline in completing 10 full attempts with careful comparison is itself a stronger readiness signal than any mock could provide, and saving the mock budget for late-stage calibration after 20+ self-evaluated attempts is the financially rational choice for someone with limited preparation funds."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "The candidate hit a content-volume threshold problem rather than a method problem. SDE2-level system design prep typically needs ~200 hours of total study to reach fluency, and at 80 hours the candidate has only completed roughly 40% of the required surface area — the gaps in the Twitter design correspond to architectural patterns and component interactions they simply haven't encountered yet. The fix is to continue reading systematically, prioritizing Twitter-class problems specifically (feed generation, fanout strategies, hot-key handling), and the production skill will follow naturally as the recognition substrate becomes broad enough to support generation.",
      ...
      "The candidate's preparation was methodologically sound but undermined by performance anxiety — the transition from solo study to a live evaluative setting activates social-threat responses that degrade working memory and executive function, causing the freeze. The fix is not to change the study method but to add graduated exposure: first, practice verbalizing designs alone while recording audio; then present to a friend in a low-stakes setting; finally, do a formal mock interview. The knowledge is present but the retrieval pathway is blocked by anxiety-driven cognitive interference.",
      "The candidate didn't create sufficiently structured memory cues for the components they studied. Reading and summarizing from memory builds propositional knowledge (understanding relationships between concepts) but not the procedural chunking needed to retrieve specific components on demand. The fix is to create and drill a mnemonic checklist of components (load balancer, cache, queue, database, CDN, message broker) and practice reciting it at the start of every design attempt so that retrieval of the component vocabulary becomes automatic, freeing working memory for the actual design reasoning."
    ],
    "correctIndex": 1,
  ...
  },
  {
    ...
    "options": [
      "Compress the timeline by reading the foundation material at 3x speed and doing twice as many practice questions per day while maintaining the standard loop ordering. The standard model's sequence is correct — foundation, then practice, then mocks — so the candidate should execute the same pipeline faster rather than restructuring it, because each stage builds on the previous one and skipping or shrinking any stage compromises the stages that follow it. At 4-5 practice problems per day and 2 mocks per week, the full pipeline fits into 3 weeks with discipline.",
      "Skip the foundation entirely and spend all 3 weeks doing back-to-back mock interviews with experienced engineers. Mocks are the most realistic signal because they combine time pressure, social evaluation, and real-time feedback in a way that solo practice cannot replicate. Maximizing the mock count maximizes the candidate's exposure to the actual evaluation environment, and the feedback from each mock serves as both the learning event and the calibration event simultaneously, eliminating the need for a separate foundation or solo-practice phase.",
      ...
      "Cancel the interview and reschedule for 3 months out. The standard prep model assumes at minimum 6-8 weeks of sequential skill-building where each stage requires the previous stage's output as input — foundation builds vocabulary, practice builds production fluency from that vocabulary, and mocks calibrate that fluency against external standards. Three weeks is structurally insufficient for this sequential dependency chain, and any compressed approach will produce superficial pattern-matching rather than the genuine architectural reasoning that senior interviewers are trained to distinguish from memorized templates."
    ],
    "correctIndex": 2,
